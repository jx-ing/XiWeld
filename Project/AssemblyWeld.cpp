...
BEGIN_MESSAGE_MAP(CAssemblyWeld, CDialog)
	...
	ON_CBN_SELCHANGE(IDC_COMBO_TABLE_GROUP3, &CAssemblyWeld::OnCbnSelchangeComboTableGroup3)
	ON_NOTIFY(TCN_SELCHANGE, IDC_TAB1, &CAssemblyWeld::OnTcnSelchangeTab1)
	ON_WM_SIZE()
END_MESSAGE_MAP()
...
BOOL CAssemblyWeld::OnInitDialog()
{
	...
	m_comboScanTimes.ResetContent();
	for (int i = 1; i <= PARA_RECOGNITION(nMaxLineScanTimes); i++)
	{
		m_comboScanTimes.AddString(std::to_string(i).c_str());
	}
	...
	// ljx 标签页 初始化
	{
		...
		// 2. 加 3 个标签
		m_tab.InsertItem(0, _T("构件"));
		m_tab.InsertItem(1, _T("CAM"));
		m_tab.InsertItem(2, _T("焊接"));

		// 3. 创建 3 个子对话框，父窗口都给 tab
		m_pageScan = new CShowCAM;
		m_pageWeld = new CWeldPage;
		m_pagePieceType = new WorkPieceType;

		m_pageScan->Create(IDD_PAGE_CAM, &m_tab);
		...
		m_pagePieceType->SetWindowPos(NULL, rc.left, rc.top, rc.Width(), rc.Height(), SWP_SHOWWINDOW);  // 默认显示第0页
	}
...
HBRUSH CAssemblyWeld::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
	...
	if (pWnd->GetDlgCtrlID() == IDC_STATIC_1007 || pWnd->GetDlgCtrlID() == IDC_STATIC_1009
		...
		|| pWnd->GetDlgCtrlID() == IDC_STATIC_1024
...
void CAssemblyWeld::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_COMBO_TABLE_GROUP3, m_comboScanTimes);
}
...
{
	image_process::VertWeldSearch cVertWeldSearch;
	cVertWeldSearch.makesureDataFolderExist();
	cVertWeldSearch.deleteOldData();
	//image_process::GenericImage cTeachImage;
...
bool CAssemblyWeld::ScanLine(int nRobotNo)
{
	...
	if (1)
	{
		...
		for (int i = 0; i < PARA_RECOGNITION(nLineScanTimes); i++)
		{
			...
			{
				sNewFolderName.Format(sFolderName);
			}
			else
			{
			sNewFolderName.Format(sFolderName + _T("%d"), i);
			}
			CLineScanForGantry cLineScanForGantry(&m_bNaturalPop);
			if (!cLineScanForGantry.LineScan(pUnit, pUnit->m_vpImageCapture, "Table", sNewFolderName)
			...
			if (vtPointCloud.size() > 0)
			{
				...
				if (i == 0)
					fprintf(file, "%4d %12.3lf %12.3lf %12.3lf ",
						0, vtPointCloud[0].x, vtPointCloud[0].y, vtPointCloud[0].z);
				else
					fprintf(file, "\n%4d %12.3lf %12.3lf %12.3lf ",
						0, vtPointCloud[0].x, vtPointCloud[0].y, vtPointCloud[0].z);
			}
			for (size_t j = 1; j < vtPointCloud.size(); j++)
			{
				fprintf(file, "\n%4zd %12.3lf %12.3lf %12.3lf ",
					j, vtPointCloud[j].x, vtPointCloud[j].y, vtPointCloud[j].z);
			}
			fclose(file);
		}
	}
...
bool CAssemblyWeld::WorkWeldAfterMeasure(int nRobotNo, int& nCurGroupNo)
{
	...
	for (int nGroupNo = pWeldAfterMeasure->m_nPauseGroupNo; nGroupNo < nWeldGropuNum; nGroupNo++)
	{
		...
		{
			if ((pRobotDriver->m_nRobotInstallDir == 1 && pRobotDriver->GetCurrentPos(ROBOT_AXIS_Z) < PARA_FLAT_MEASURE(dSafeHeight))
				|| (pRobotDriver->m_nRobotInstallDir == -1 && pRobotDriver->GetCurrentPos(ROBOT_AXIS_Z) > PARA_FLAT_MEASURE(dSafeHeight))
				)
			{
		...
		if (/*(!pWeldAfterMeasure->m_bWorkpieceShape || dDis <= 1.0) && */!pWeldAfterMeasure->CalcWeldTrack(nGroupNo))
		{
			...
			{
				//已修改
				XUI::MesBox::PopInfo("{0}立缝“计算”失败，继续其他焊缝焊接", pRobotDriver->m_strRobotName.GetBuffer());
				//continue;
			}
			return false;
		}
		...
		if (m_dCurWeldLenBeforeCleanGun > PARA_SYSTEM(dMaxWeldLengthForCleanGun)) // 大于3米进行一次清枪
		{
...
void CAssemblyWeld::LoadTableGroupScan()
{
	...
	SetCurLineScanFolder(nCurTableNo);
...
//	}
//
//	auto pImg = cvLoadImage("Pics/Src_4.jpg", 0);
//
//	WidgetLaserInfo* LaserInfo = new WidgetLaserInfo[10];
//	char* LogName = "WidgetLaserLock";
...
//	if (true)
//	{
//		auto bImageDirection = 1;// PARA_FLAT_MEASURE(bImageDirection);
//		auto bCrossFilp = 0;// PARA_FLAT_MEASURE(bCrossFilp);
//		int PtnNum = WidgetLaserLock(pImg, LaserInfo, bImageDirection, bCrossFilp, LogName);
//		//bool state = WidgetLaserTrack(pImg, LaserInfo,"WidgetLaserTrack-corner");
//		if (PtnNum <= 0)
//		{
//			XUI::MesBox::PopInfo("示教失败FF，请检查激光图片是否正常");
//		}
//		else if (PtnNum >= 1)
//		{
...
//				}
//				if (1 < vLeftPtn.size() && 1 < vRightPtn.size()) // 或 改 且 兼容E_L_LINE_POINT和E_R_LINE_POINT
//				{
//					IplImage* pColorImg = cvCreateImage(cvSize(pImg->width, pImg->height), IPL_DEPTH_8U, 3);
//					cvCvtColor(pImg, pColorImg, CV_GRAY2RGB);
//					cvCircle(pColorImg, tKeyPoint, 10, CV_RGB(255, 0, 0), 8);
//					cvLine(pColorImg, vLeftPtn[0], vLeftPtn.back(), CV_RGB(0, 255, 0), 4);
//					cvLine(pColorImg, vRightPtn[0], vRightPtn.back(), CV_RGB(0, 0, 255), 4);
...
bool CAssemblyWeld::LoadCloudProcessResultMultiMachine(CString sFileName/* = ""*/)
{
  ...
  void CAssemblyWeld::ShowTeachImageOn(CWnd* pParent, const std::vector<int>& drawIds)
  {
	  ...
	  CDialog* pDlg = static_cast<CDialog*>(pParent);   // 关键：改成 CDialog*

	  const int nBuffSize = (int)m_vpShowLaserImgBuff.size();
	  const int nDraw = (int)drawIds.size();
	  for (int i = 0; i < nDraw && i < nBuffSize; ++i) {
		  IplImage* pImg = m_vpShowLaserImgBuff[i];
		  if (!pImg || pImg->imageData[0] < 0) continue;

		  const int ctrlId = drawIds[i];
		  DrawImage(pImg, pDlg, ctrlId, /*bDrawCross=*/false);
	  }
  }

  void CAssemblyWeld::ShowTeachImageFirstOn(CWnd* pParent, int ctrlId, bool bDrawCross /*=false*/)
  {
	  if (!pParent || !::IsWindow(pParent->m_hWnd)) return;
	  if (m_vpShowLaserImgBuff.empty()) return;
...
//	 fclose(CoutTeachPointRobot);
 ...
 void CAssemblyWeld::OnCbnSelchangeComboTableGroup2()
 {
	 ...
	 opini.SetFileName(DATA_PATH + m_vpRobotDriver[0]->m_strRobotName + LINE_SCAN_PARAM);
	 opini.SetSectionName("CurUseTableNo");
	 opini.ReadString("CurUseTableNo", &nWorkTableNo);
	 int nCurSel = m_comboTableGroupleft.GetCurSel();
	 if (nWorkTableNo == nCurSel)
	 {
		 return;
	 }
	 ...
	 {
		 XUI::MesBox::PopError(_T("设置工作台失败，无效的工作台编号{0}！"), nCurSel);
 ...
 */
 bool CAssemblyWeld::WeldSchedule(WeldAfterMeasure* pWeldData, int nGroupNo)
 {
	 ...
	 for (int i = pWeldData->m_nPauseWeldNo; i < 8; i++) {
		 ...
		 T_ROBOT_COORS tIdealEndPnt = vtRealWeldCoord.back();
		 ...
		 if (pWeldData->m_vvtWeldLineInfoGroup[nGroupNo][nWeldNo].tAtrribute.bWeldMode)
		 {
			 ...
			 tTraceWeldObj.m_pScanInit->m_pFlatWeldRealTimeTrack->setShowImage(tTraceWeldObj.m_pColorImg);
			 tTraceWeldObj.m_pScanInit->m_pFlatWeldRealTimeTrack->setHardware(pWeldData->m_ptUnit, pWeldData->m_ptUnit->m_nTrackCameraNo);
			 tTraceWeldObj.m_pScanInit->m_pFlatWeldRealTimeTrack->createImageBuffer();
			 tTraceWeldObj.m_pScanInit->m_pFlatWeldRealTimeTrack->deleteOldData();
			 ...
			 for (int nLayerNo = pWeldData->m_nPauseLayerNo; nLayerNo < vtWeldPara.size(); nLayerNo++)
			 {
				 ...
				 {
					 bDoWeldSuccess = tTraceWeldObj.DoweldRuning(tTraceWeldObj.m_pRobotDriver,
				 ...
				 {
					 bDoWeldSuccess = tTraceWeldObj.DoWeldRuningEstun(tIdealEndPnt, nGroupNo, tTraceWeldObj.m_pRobotDriver,
			 ...
			 for (int nLayerNo = pWeldData->m_nPauseLayerNo; nLayerNo < vtWeldPara.size(); nLayerNo++)
			 {
				 ...
				 {
					 tTraceWeldObj.m_ptUnit->SwitchIO("TrackLensProtection", false);
 ...
 void CAssemblyWeld::setWorkpieceType(int nTypeNo)
 {
	 ...
	 sSrcFile = ".\\ConfigFiles\\WorkpieceType\\" + m_vsWorkpieceType[nTypeNo] + "\\RTT.ini";
 ...
 void CAssemblyWeld::OnCbnSelchangeComboTableGroup3()
 {
	 UpdateData(TRUE);
	 int nScanTimes = m_comboScanTimes.GetCurSel() + 1;
	 SetLineScanTimes(nScanTimes);
	 WriteLog("选择扫描次数：%d", nScanTimes);
 }
 ...
 void CAssemblyWeld::OnTcnSelchangeTab1(NMHDR* pNMHDR, LRESULT* pResult)
 {
	 ...
	 case 0: // 构件
		 ...
		 m_pageScan->DeactivatePage();   // 离开 CAM
		 break;
	 case 1: // CAM
		 place(m_pageScan);
 ...
 void CAssemblyWeld::CreateTabBlock()
 {
	 if (::IsWindow(m_hBlockTop))
		 return;

	 // 计算 tab 的内容区（去掉 tab 头）
	 CRect rcTab;
	 m_tab.GetClientRect(&rcTab);
	 m_tab.AdjustRect(FALSE, &rcTab);

	 m_hBlockTop = ::CreateWindowEx(
		 0,                  // 不需要 TOPMOST 了，child 会被裁剪在 tab 内
		 kCamBlockClass,
		 _T(""),
		 WS_CHILD | WS_VISIBLE,
		 rcTab.left,
		 rcTab.top,
		 rcTab.Width(),
		 rcTab.Height(),     // 如果只想挡一部分，可以自己改成 coverHeight
		 m_tab.GetSafeHwnd(),// ★ 父窗口 = tab 控件
		 nullptr,
 ...
 void CAssemblyWeld::OnSize(UINT nType, int cx, int cy)
 {
	 ...
	 ::MoveWindow(m_hBlockTop, rcTab.left, rcTab.top, rcTab.Width(), rcTab.Height(), TRUE);
 }
