...
void ChangeScanLineParam::OnBnClickedBtnok()
{
	...
	//添加给新线扫
	int nFileNo = 0;
	CString sFile = PARA_RECOGNITION(sLineScanFolder) + "0\\Table.ini";
	if (PARA_RECOGNITION(nLineScanTimes) == 1)
	{
		sFile = PARA_RECOGNITION(sLineScanFolder) + "\\Table.ini";
		//写入距离
		opini.SetFileName(sFile);
		opini.SetSectionName("Base");
#ifndef SINGLE_ROBOT
		opini.WriteString("MoveVector_BX", m_ScanEndLocation > m_ScanStartLocation ? 1 : -1);
		opini.WriteString("MoveVector_BY", 0.0);
		opini.WriteString("MoveVector_BZ", 0.0);
#else
		opini.WriteString("MoveVector_BX", 0.0);
		opini.WriteString("MoveVector_BY", m_ScanEndLocation > m_ScanStartLocation ? 1 : -1);
		opini.WriteString("MoveVector_BZ", 0.0);
#endif
		opini.SetSectionName("Section0");
#ifndef SINGLE_ROBOT
		opini.WriteString("LineScanStartPos_BX", m_ScanStartLocation);
		opini.WriteString("LineScanStartPos_BY", 0.0);
		opini.WriteString("LineScanStartPos_BZ", 0.0);
#else
		opini.WriteString("LineScanStartPos_BX", 0.0);
		opini.WriteString("LineScanStartPos_BY", m_ScanStartLocation);
		opini.WriteString("LineScanStartPos_BZ", 0.0);
#endif
		opini.WriteString("ScanDis", abs(m_ScanEndLocation - m_ScanStartLocation));
	}
	else
	{
	while (CheckFileExists(sFile, false))
	{
		//写入距离
		opini.SetFileName(sFile);
		opini.SetSectionName("Base");
		...
		//交换起终点
		std::swap(m_ScanStartLocation, m_ScanEndLocation);

		//更新文件名
		nFileNo++;
		sFile.Format(PARA_RECOGNITION(sLineScanFolder) + "%d\\Table.ini", nFileNo);
	}
	}
	return;
}
...
void ChangeScanLineParam::OnBnClickedBtnok4()
{
	if (false == m_bIsRunning)
	{
		...
		if (0 != m_pUnit->MoveExAxisFun(m_ScanStartLocation, m_pUnit->GetRobotCtrl()->m_tPulseHighSpeed.dSpeed*12, nAxisNo, dMaxExAxisSpeed))
		{
			m_bIsRunning = false;
			return;
		}
...
void ChangeScanLineParam::OnBnClickedBtnok5()
{
	if (false == m_bIsRunning)
	{
		...
		int nAxisNo = m_pUnit->m_nLinedScanAxisNo;
		double dMaxExAxisSpeed = m_pUnit->GetMotorCtrl()->GetMaxSpeed(m_pUnit->XYZAxisNoToSoftAxisNo(nAxisNo));
		if (0 != m_pUnit->MoveExAxisFun(m_ScanEndLocation, m_pUnit->GetRobotCtrl()->m_tPulseHighSpeed.dSpeed*12, nAxisNo, dMaxExAxisSpeed))
		{
			m_bIsRunning = false;
			return;
		}
