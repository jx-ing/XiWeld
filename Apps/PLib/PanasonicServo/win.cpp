...
{
  static string windows_com_port_prefix = "\\\\.\\";
  if (input.compare(windows_com_port_prefix) != 0)
  {
    return windows_com_port_prefix + input;
  }
...
#include <fstream>
std::ofstream fout_serial_open("Serial_Open.txt");
void
Serial::SerialImpl::open ()
{
  if (port_.empty ()) {
	  fout_serial_open << "Empty port is invalid.";
    throw invalid_argument ("Empty port is invalid.");
  }
  if (is_open_ == true) {
	  fout_serial_open << "Serial port already open.";
  ...
  if (fd_ == INVALID_HANDLE_VALUE) {
    ...
    case ERROR_FILE_NOT_FOUND:
	  ...
	  fout_serial_open << ss.str().c_str() << std::endl;
      THROW (IOException, ss.str().c_str());
    default:
      ss << "Unknown error opening the serial port: " << errno;
	  fout_serial_open << port_ << std::endl << errno_ << std::endl;
      fout_serial_open << ss.str().c_str() << std::endl;
	  THROW (IOException, ss.str().c_str());
    }
  }
...
bool
Serial::SerialImpl::waitReadable (uint32_t1 /*timeout*/)
{
...
size_t
Serial::SerialImpl::read (uint8_t1 *buf, size_t size)
{
  if (!is_open_) {
...
size_t
Serial::SerialImpl::write (const uint8_t1 *data, size_t length)
{
  if (is_open_ == false) {
