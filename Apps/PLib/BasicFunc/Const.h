...
#include "Apps/SmallPiece/Infrastructure/Math/BasicCalc.h"
...
const CString LINE_SCAN_POINT_CLOUD				=	_T(".\\LineScan\\AllPointCloud.txt");
...
double TwoPointDis3D(T1 t1, T2 t2)
{
	double dX = t2.x - t1.x;
	double dY = t2.y - t1.y;
	double dZ = t2.z - t1.z;
...
template<class T1, class T2>
double TwoPointDis2D(T1 t1, T2 t2)
{
	double dX = t2.x - t1.x;
	double dY = t2.y - t1.y;
