...
void CRobotDriverAdaptor::LoadSpeedAndSafeHeight()
{
	...
	opini.SetSectionName("BackGunSpeed");
	opini.ReadString("Speed", &m_tBackGunSpeed.dSpeed);
	opini.ReadString("Acc", &m_tBackGunSpeed.dACC);
	opini.ReadString("Dec", &m_tBackGunSpeed.dDEC);
	opini.SetSectionName("SpeedForMoveToLineScanBeginPos");
	opini.ReadString("Speed", &m_tSpeedForMoveToLineScanBeginPos.dSpeed);
	opini.ReadString("Acc", &m_tSpeedForMoveToLineScanBeginPos.dACC);
	opini.ReadString("Dec", &m_tSpeedForMoveToLineScanBeginPos.dDEC);
}
...
bool CRobotDriverAdaptor::MoveToSafeHeight()
{
	XUI::MesBox::PopError(_T("MoveToSafeHeight仅限埃斯顿机器人可用"));
	return false;
	//double dRobotInstallMode = m_nRobotInstallDir;
	...
	//tCurPulse = GetCurrentPulse();
	//tCurCoord = GetCurrentPos();
	//RobotKinematics(m_tHomePulse, m_tTools.tGunTool, tHomeCoord);
	//RobotKinematics(tCurPulse, m_tTools.tGunTool, tTransCoord);
	...
	//	}
	//	tRobotMoveInfo = PVarToRobotMoveInfo(0, tTempCoord, m_tCoordHighSpeed, MOVL);
...
bool CRobotDriverAdaptor::EstunTrackInit(const T_ROBOT_COORS& startPos, double dTrackCurrent, double dTrackVoltage ,const CString& IP,double Speed, const vector<int>& Cfg)
{
	return false;
}
