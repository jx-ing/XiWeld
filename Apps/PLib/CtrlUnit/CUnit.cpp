...
bool CUnit::CheckRobotDone(T_ROBOT_COORS tAimCoor)
{
...
bool CUnit::CheckRobotDone(T_ANGLE_PULSE tAimPulse)
{
	if (g_bLocalDebugMark)
	{
		return true;
...
bool CUnit::TeachMove(const std::vector<T_ANGLE_PULSE>& vtMeasurePulse, double dExAxlePos, const std::vector<int>& vnMeasureType, int nTrigSigTime)
{
	...
	if (0 != MoveExAxisFun(dExAxlePos, PARA_FLAT_MEASURE(dExAxisPosMoveSpeed), nAxisNo))
		return false;
	WorldCheckRobotDone();
	// 需要更改获取轴数据，临时更改
