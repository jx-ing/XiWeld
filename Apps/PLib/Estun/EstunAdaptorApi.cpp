...
#include "stdafx.h"
#include "EstunAdaptorApi.h"
#include ".\Project\XiGrooveRobot.h"


EstunAdaptorApi::EstunAdaptorApi(CString strUnitName, CLog* cLog, std::vector<CUnitDriver*>* ppUnitDriver)
...
EstunAdaptorApi::~EstunAdaptorApi()
{
}

bool EstunAdaptorApi::getWelderMode(E_WELD_SEAM_TYPE eWeldSeamType, CString& sWelderMode_W, CString& sWelderMode_MM)
{
	...
	case E_PLAT_GROOVE:
		switch (PARA_ESTUN(nFlatWeldMode))
		{
		case 0:
			sWelderMode_W = PARA_ESTUN(sFlatNormalWelderMode_W);
			sWelderMode_MM = PARA_ESTUN(sFlatNormalWelderMode_MM);
			return true;
		case 1:
			sWelderMode_W = PARA_ESTUN(sFlatPulseWelderMode_W);
			sWelderMode_MM = PARA_ESTUN(sFlatPulseWelderMode_MM);
			return true;
		case 2:
			sWelderMode_W = PARA_ESTUN(sFlatDualPulseWelderMode_W);
			sWelderMode_MM = PARA_ESTUN(sFlatDualPulseWelderMode_MM);
			return true;
		}
		XUI::MesBox::PopError("不存在的平焊焊接模式：{0}", PARA_ESTUN(nFlatWeldMode));
		return false;
	case E_STAND_SEAM:
	case E_STAND_MULTIPLE:
	case E_STAND_GROOVE:
		switch (PARA_ESTUN(nVerWeldMode))
		{
		case 0:
			sWelderMode_W = PARA_ESTUN(sVerNormalWelderMode_W);
			sWelderMode_MM = PARA_ESTUN(sVerNormalWelderMode_MM);
			return true;
		case 1:
			sWelderMode_W = PARA_ESTUN(sVerPulseWelderMode_W);
			sWelderMode_MM = PARA_ESTUN(sVerPulseWelderMode_MM);
			return true;
		case 2:
			sWelderMode_W = PARA_ESTUN(sVerDualPulseWelderMode_W);
			sWelderMode_MM = PARA_ESTUN(sVerDualPulseWelderMode_MM);
			return true;
		}
		XUI::MesBox::PopError("不存在的立焊焊接模式：{0}", PARA_ESTUN(nVerWeldMode));
		return false;
	}
...
//清除报警信息+
void  EstunAdaptorApi::cleanAlarm()
{
	int sRt;
...
void EstunAdaptorApi::SetSysMode(int mode)
{
	int sRt;
...
void EstunAdaptorApi::SetTpSpeed(int speed)//设置TP示教器上的速度%
{
	int sRt;
	sRt = m_pEstunRobot->SetTpSpeed_Py(speed);
}
void EstunAdaptorApi::LoadUserProgramer(const char* projName, const char* progName)
{
	api.E_LoadProgame(projName, progName);
}
//设置当前工具 !+
bool EstunAdaptorApi::SetRobotToolNo(int nToolNo)
{
	CString ToolName;
...
//查询机器人当前各轴角度(暂无外部轴) +
double EstunAdaptorApi::GetCurrentDegree(int nAxisNo)
{
	double adPos[10] = { 0.0 };
...
//获取工具信息	+		
BOOL EstunAdaptorApi::GetToolData(UINT unToolNo, double adRobotToolData[6])
{
	E_ROB_COORD toolCoord;
	E_ROB_PAYLOAD toolPayload;
	if (0 != api.E_GetTool(unToolNo, toolCoord, toolPayload))
	{
		return FALSE;
	}

	adRobotToolData[0] = toolCoord.getX();
	adRobotToolData[1] = toolCoord.getY();
	adRobotToolData[2] = toolCoord.getZ();
	adRobotToolData[3] = toolCoord.getC();
	adRobotToolData[4] = toolCoord.getB();
	adRobotToolData[5] = toolCoord.getA();
	return TRUE;
}

void EstunAdaptorApi::SetPosVar(int nIndex, T_ROBOT_COORS tRobotCoors, int config[7], int scoper)
{
	char name[30];
	memset(name, '\0', 30);
	sprintf_s(name, "mP%d", nIndex);

	E_ROB_POS pos(tRobotCoors.dX, tRobotCoors.dY, tRobotCoors.dZ, tRobotCoors.dRZ, tRobotCoors.dRY, tRobotCoors.dRX, tRobotCoors.dBX, tRobotCoors.dBY);

	if (api.E_WritePos(name, project, pos))
	{
		XUI::MesBox::PopOkCancel("埃斯顿机器人{0}设置直角坐标变量失败", m_nRobotNo);
	}
}
void EstunAdaptorApi::SetPosVar(int nIndex, T_ANGLE_PULSE tRobotPulse, int scoper)
{
	char name[30];
	memset(name, '\0', 30);
	sprintf_s(name, "mJP%d", nIndex);

	E_ROB_POS pulse(tRobotPulse.nSPulse * m_tAxisUnit.dSPulse, tRobotPulse.nLPulse * m_tAxisUnit.dLPulse,
	...
	if (api.E_WritePos(name, project, pulse))
	{
		XUI::MesBox::PopOkCancel("埃斯顿机器人{0}设置关节坐标变量失败", m_nRobotNo);
	}
}

// 埃斯顿机器人专用: 速度变量读写
void EstunAdaptorApi::SetMultiSpeed(UINT index, UINT count, double** speedVar)//设置大量速度
{
	m_pEstunRobot->SetMultiSpeed(index, count, speedVar);
}
void EstunAdaptorApi::SetSpeed(int nIndex, double adSpeed[5]) // 设置单个速度
{
	m_pEstunRobot->SetSpeed(nIndex, adSpeed);
}
void EstunAdaptorApi::SetWeaveDate(const char* name, ESTUN_WeaveDate WeaveDate, int scope)
{
...
int EstunAdaptorApi::GetIntVar(int nIndex, char* cStrPreFix)
{
	int nValue;
...
void EstunAdaptorApi::SetIntVar(int nIndex, int nValue, int score, char* cStrPreFix)
{
	char cIntVarName[100] = { 0 };
	sprintf(cIntVarName, "%s%d", cStrPreFix, nIndex);
	//m_pRobotCtrl_Estun->GetIntVar(cIntVarName, nValue);
...
//设置Real变量
bool EstunAdaptorApi::SetRealVar(int nIndex, double value, char* cStrPreFix, int score)
{
	char cIntVarName[100] = { 0 };
	sprintf(cIntVarName, "%s%d", cStrPreFix, nIndex);
...
int EstunAdaptorApi::AxisPulseMove(int nAxisNo, long lDist, long lRobotSpd, WORD wCoorType, int nToolNo, long lCoordFrm)
{
	double dDist = lDist;
	dDist /= 2000; //脉冲转换为度数
	if (lCoordFrm == 0)
...
void EstunAdaptorApi::PosMove(int nAxisNo, double dDist, long lRobotSpd, WORD wCoorType, int nToolNo, long lCoordFrm)
{
	int config[7]{0};
...
//通用移动函数
void EstunAdaptorApi::MoveByJob(double* dRobotJointCoord, T_ROBOT_MOVE_SPEED tPulseMove, int nExternalAxleType, int nPVarType, CString JobName, int config[7])
{
	if (NULL == config)
	{
		for (int i = 0; i < 7; i++)
...
bool EstunAdaptorApi::SetOutIOVar(int name, int value)
{
...
bool EstunAdaptorApi::GetInputIOVar(int name, int& value)
{
...
// 埃斯顿角度坐标专用函数（只能关节插补）
void EstunAdaptorApi::MoveByJobForAngle(double* adRobotAngleCoord, T_ROBOT_MOVE_SPEED tPulseMove, CString JobName)
{
	double dPos[9] = { 0,0,0,0,0,0,0,0,0 };
...
bool EstunAdaptorApi::EstunSendTrackData(const vector<T_ROBOT_COORS>& WeldPathPoint, const vector<vector<int>>& Cfg)
{
...
bool EstunAdaptorApi::EstunTrackInit( const T_ROBOT_COORS& startPos,double dTrackCurrent, double dTrackVoltage, const CString& IP,  double Speed, const vector<int>& Cfg)
{
	ESString myIp(IP);
	...
	E_ROB_POS EstartPos(cfgtmp22, startPos.dX, startPos.dY, startPos.dZ + 2.5 * m_nRobotInstallDir, startPos.dRZ, startPos.dRY, startPos.dRX, startPos.dBX, startPos.dBY, 0, 0, 0, 0, 0, 0, 0, 0);
	

	//埃斯顿发送电流电压
	if (m_eRobotBrand == ROBOT_BRAND_ESTUN)
	{
		m_pEstunRobot->SetIntVar_Py("INT1",  1);
		m_pEstunRobot->SetIntVar_Py("INT6", PARA_ESTUN(nFlatWeldMode));
		m_pEstunRobot->SetRealVar_Py("WELD_L", dTrackCurrent);
		m_pEstunRobot->SetRealVar_Py("WELD_V", dTrackVoltage);
	}
	...
	double dSpeed[5];
	dSpeed[0] = 20.0;
	dSpeed[1] = Speed / 60.0;//换算到mm/s
	dSpeed[2] = 360.0;
	dSpeed[3] = Speed / 60.0;//换算到mm/s
	dSpeed[4] = 360.0;
	m_pEstunRobot->SetSpeed("SPEEDWELD", dSpeed);
...
bool EstunAdaptorApi::EstunTrackStop()
{
...
bool EstunAdaptorApi::EstunClearWeldBuffer()
{
...
//调用手操盒程序 
int EstunAdaptorApi::CallJob(char JobName[24])
{
	m_cLog->Write("CallJob:%s", JobName);
	if (!api.E_LoadProgame("Xi_Robot", JobName))
	{
		XUI::MesBox::PopError("示教器程序 {1} 加载失败", JobName);
		return -2; //加载不成功
	};

	if (!api.E_ProgRun())
	{
		XUI::MesBox::PopError("示教器程序 {1} 运行失败", JobName);
		return -1; //加载不成功
	}
	return 0;
}

int EstunAdaptorApi::CallJob(CString sJobName)
{
	m_cLog->Write("CallJob:%s", sJobName);
	if (!api.E_LoadProgame("Xi_Robot", sJobName.GetBuffer()))
	{
		XUI::MesBox::PopError("示教器程序 {1} 加载失败", sJobName);
		return -2; //加载不成功
	};

	if (!api.E_ProgRun())
	{
		XUI::MesBox::PopError("示教器程序 {1} 运行失败", sJobName);
		return -1; //加载不成功
	}
	return 0;
}
...
//直角坐标运动函数
void EstunAdaptorApi::MoveByJob(T_ROBOT_COORS tRobotJointCoord, T_ROBOT_MOVE_SPEED tPulseMove, int nExternalAxleType, CString JobName, int config[7])////默认 mode值Cf值为零
{
	if (NULL == config)
	{
		int cnfg[7] = { 0 };
...
//关节坐标运动函数
void EstunAdaptorApi::MoveByJob(T_ANGLE_PULSE tRobotJointCoord, T_ROBOT_MOVE_SPEED tPulseMove, int nExternalAxleType, CString JobName)
{
	// Pulse => Angle
...
T_ROBOT_MOVE_INFO EstunAdaptorApi::PVarToRobotMoveInfo(int nVarNo, T_ANGLE_PULSE tPulse, T_ROBOT_MOVE_SPEED tSpeed, int nMoveType, UINT unToolNum, UINT unUserNo, UINT unPosture)
{
...
T_ROBOT_MOVE_INFO EstunAdaptorApi::PVarToRobotMoveInfo(int nVarNo, T_ROBOT_COORS tCoord, T_ROBOT_MOVE_SPEED tSpeed, int nMoveType, UINT unToolNum, UINT unUserNo, UINT unPosture)
{
...
void EstunAdaptorApi::CalModeValue(T_ANGLE_PULSE tReferPluse, const char* RobotMode, int config[7])
{
	double Axis[6];
...
bool EstunAdaptorApi::SetMoveValue(std::vector<T_ROBOT_MOVE_INFO> vtMoveInfo, bool bSafeMove, bool bUsePB)
{
...
int EstunAdaptorApi::MoveExAxisForLineScan(double dDist, int nSpeed) {
	int config[7] = { 0 };
...
bool EstunAdaptorApi::MoveToSafeHeight() {
...
bool EstunAdaptorApi::TeachMove(const std::vector<T_ANGLE_PULSE>& vtMeasurePulse, double dExAxlePos, const  std::vector<int>& vnMeasureType, int nTrigSigTime)
{

	//#ifndef SINGLE_ROBOT
	// 外部轴运动
	if (0 != MoveExAxisForLineScan(dExAxlePos, PARA_FLAT_MEASURE(dExAxisPosMoveSpeed)))
	{
		return false;
	}
	WorldCheckRobotDone();
...
//bool EstunAdaptorApi::CleanGunH()
//{
//	//位置过低时先抬枪再回到安全位置
...
//	return true;
//}
int EstunAdaptorApi::MoveToLineScanPos(T_ANGLE_PULSE tPulse, T_ROBOT_MOVE_SPEED tPulseMove)
{
...
int EstunAdaptorApi::ContiMoveAny(const std::vector<T_ROBOT_MOVE_INFO>& vtRobotMoveInfo)
{
...
bool EstunAdaptorApi::CalcWeaveWeldTrack(std::vector<T_ROBOT_COORS>& vtWeldTrack, const T_WELD_PARA& tWeldPara, E_WELD_SEAM_TYPE eWeldSeamType, double& dRealWeldSpeed)
{
...
int EstunAdaptorApi::WeldMoveRobotWeave(const std::vector<T_ROBOT_COORS>& vtWeldPathPoints, const  std::vector<int>& vnPtnType, const T_WELD_PARA& tWeldPara, double dExAxlePos, E_WELD_SEAM_TYPE eWeldSeamType, bool bIsArcOn)
{
	...
	if (bHasWrap && bHasStand)
	{
		SetIntVar(7, bHasStand, 2);  // s -> ms
	}
	else
	{
		SetIntVar(7, !bHasStand, 2);  // s -> ms
	...
	{
		SetIntVar(6, PARA_ESTUN(nFlatWeldMode)/*tWeldPara.nWeldMethod*/);
	}
...
bool EstunAdaptorApi::SendWeldProgram(int nWeldTrackNum, E_WELD_SEAM_TYPE eWeldSeamType)
{
	...
	fprintf(pf, "    ARC_SET{M=\"DIRECT\",A=t_g.REAL0,V=t_g.REAL1,H=0}\n");
	fprintf(pf, "    ARC_ON{}\n");
	fprintf(pf, "    Wait{T=t_p.INT4}\n");
	fprintf(pf, "ENDIF\n");
	fprintf(pf, "/*Weld*/\n");
	fprintf(pf, "IF t_p.INT2.value == 1 THEN\n");
	fprintf(pf, "    ARC_SET{M=\"DIRECT\",A=t_g.REAL2,V=t_g.REAL3,H=0}\n");
	...
	fprintf(pf, "    ARC_SET{M=\"DIRECT\",A=t_g.REAL4,V=t_g.REAL5,H=0}\n");
	...
	fprintf(pf, "    ARC_ON{}\n");
	fprintf(pf, "ENDIF\n");
	fprintf(pf, "IF t_p.INT7.value == 0 THEN\n");
	fprintf(pf, "Wait{T = 1000}\n");
	fprintf(pf, "    ARC_StichL{startPos=t_l.P499,endPos=t_l.P500,V=t_l.S1,B=\"FINE\",T=500,mode=\"GAP\",G=1}\n");
	fprintf(pf, "ENDIF\n");
	//fprintf(pf, "IF t_p.INT1.value == 1 THEN\n");
	fprintf(pf, "IF t_p.INT7.value == 1 THEN\n");
	...
	fprintf(pf, "/*ArcOff*/\n");
	fprintf(pf, "IF t_p.INT1.value == 1 THEN\n");
	fprintf(pf, "    ARC_SET{M=\"DIRECT\",A=t_g.REAL6,V=t_g.REAL7,H=0}\n");
	...
	// 上传程序文件
	if(api.E_uploadProgFile("192.168.60.63", sLocalFile.GetBuffer(),"XiRobot", "WELD"))
	{
...
int EstunAdaptorApi::WeldMoveTriangleWeave(const std::vector<T_ROBOT_COORS>& vtWeldPathPoints, const  std::vector<int>& vnPtnType, const T_WELD_PARA& tWeldPara, double dExAxlePos, E_WELD_SEAM_TYPE eWeldSeamType, bool bIsArcOn)
{
	// 工程变量：INT1 INT2 INT3 INT4 INT5 : 是否起弧 是否焊接 是否包角 起弧停留 收弧停留
	...
	if (nWeldPtnNum < 0 || vtWeldPathPoints.size() > WELD_TRACK_MAX_NUM)
	{
		XUI::MesBox::PopInfo("焊接轨迹数异常 {0}", nWeldPtnNum);
		return -1;
	}
	...
	if (nWeldPtnNum >= 300)
	{
		if (false == SendWeldTriangleWeaveProgramRobot(vtWeldPathPoints.size(), eWeldSeamType))
		{
			XUI::MesBox::PopInfo("焊接程序发送失败！ 点数：{0}", nWeldPtnNum);
			return -1;
		}
	...
	{
		vtAllCoord.clear();
		vtAllCoord = vtWeldPathPoints;
	}


	T_ROBOT_COORS tLastCoord = vtAllCoord.back(); // 只有包角轨迹时会有问题
	if (0 == nWrapPtnNum)
	{
		vtWrapCoord.push_back(tLastCoord);
		vtWrapCoord.push_back(tLastCoord);
	...
	ESTUN_WeaveDate WeaveDate;
	WeaveDate.Type = 2;
	WeaveDate.DelayType_L = 0;
	WeaveDate.DelayType_C = 0;
	WeaveDate.DelayType_R = 0;
	WeaveDate.Freq = tWeldPara.tWeaveParam.dFreq;
	WeaveDate.Amp_L = tWeldPara.tWeaveParam.dAmpL;
	WeaveDate.Amp_R = tWeldPara.tWeaveParam.dAmpR;
	WeaveDate.StopTime_L = tWeldPara.tWeaveParam.dTimeL;
	WeaveDate.StopTime_C = tWeldPara.tWeaveParam.dTimeC;
	WeaveDate.StopTime_R = tWeldPara.tWeaveParam.dTimeR;
	WeaveDate.RotAngle_X = tWeldPara.tWeaveParam.dRotAngleX;
	WeaveDate.RotAngle_Z = tWeldPara.tWeaveParam.dRotAngleZ;
	SetWeaveDate("WEAVE1",WeaveDate);
...
int EstunAdaptorApi::WeldMoveLWeave(const std::vector<T_ROBOT_COORS>& vtWeldPathPoints, const std::vector<int>& vnPtnType, const T_WELD_PARA& tWeldPara, double dExAxlePos, E_WELD_SEAM_TYPE eWeldSeamType, bool bIsArcOn)
{
	// 工程变量：INT1 INT2 INT3 INT4 INT5 : 是否起弧 是否焊接 是否包角 起弧停留 收弧停留
...
int EstunAdaptorApi::WeldMove(const std::vector<T_ROBOT_COORS>& vtWeldPathPoints, const  std::vector<int>& vnPtnType, const T_WELD_PARA& tWeldPara, double dExAxlePos, E_WELD_SEAM_TYPE eWeldSeamType, bool bIsArcOn)
{
...
double EstunAdaptorApi::CalcPosture(double dPosture1, double dPosture2, double dRatio)
{
...
bool EstunAdaptorApi::SendWeldTriangleWeaveProgram(int nWeldTrackNum, E_WELD_SEAM_TYPE eWeldSeamType)
{
	CString sWelderMode_W;		//焊接模式参数W
	CString sWelderMode_MM;		//焊接模式参数MM
	if (!getWelderMode(eWeldSeamType, sWelderMode_W, sWelderMode_MM))
	...
	// 上传程序文件
	if (api.E_uploadProgFile("192.168.60.63", sLocalFile.GetBuffer(), "XiRobot", "WELD_TRIANGLE"))
	{
		XUI::MesBox::PopInfo("上传程序文件:{0} 失败！", sLocalFile);
		return false;
	}
...
bool EstunAdaptorApi::SendWeldTriangleWeaveProgramRobot(int nWeldTrackNum, E_WELD_SEAM_TYPE eWeldSeamType)
{
	CString sWelderMode_W;		//焊接模式参数W
	CString sWelderMode_MM;		//焊接模式参数MM
	if (!getWelderMode(eWeldSeamType, sWelderMode_W, sWelderMode_MM))
	...
	fprintf(pf, "    ARC_SET{M=\"DIRECT\",A=t_g.REAL4,V=t_g.REAL5,H=0}\n");
	fprintf(pf, "    SetCartDyn{Acc=100,Dec=100,Jerk=100,OriAcc=3,OriDec=3,OriJerk=100,Torq=100}\n");
	fprintf(pf, "    ARC_Line{P=t_l.P%d,V=t_l.S1,B=\"FINE\"}\n", WELD_TRACK_MAX_NUM + 1);
	fprintf(pf, "    ARC_Line{P=t_l.P%d,V=t_l.S1,B=\"FINE\"}\n", WELD_TRACK_MAX_NUM + 2);
	fprintf(pf, "    SetCartDyn{Acc=100,Dec=100,Jerk=100,OriAcc=100,OriDec=100,OriJerk=100,Torq=100}\n");
	fprintf(pf, "ENDIF\n");
	fprintf(pf, "/*ArcOff*/\n");
	fprintf(pf, "IF t_p.INT1.value == 1 THEN\n");
	...
	fprintf(pf, "    ARC_OFF{}\n");
	fprintf(pf, "ENDIF\n");
	fprintf(pf, "MovL{P=t_l.P%d,V=t_l.S1}\n", WELD_TRACK_MAX_NUM + 2);
	fprintf(pf, "End;\n");
	fclose(pf);
...
bool EstunAdaptorApi::SendWeldLWeaveProgram(int nWeldTrackNum, E_WELD_SEAM_TYPE eWeldSeamType)
{
	CString sWelderMode_W;		//焊接模式参数W
	CString sWelderMode_MM;		//焊接模式参数MM
	if (!getWelderMode(eWeldSeamType, sWelderMode_W, sWelderMode_MM))
	...
	// 上传程序文件
	if (api.E_uploadProgFile("192.168.60.63", sLocalFile.GetBuffer(), "XiRobot", "WELD_L_WEAVE"))
	{
		XUI::MesBox::PopInfo("上传程序文件:{0} 失败！", sLocalFile);
		return false;
	}
...
int EstunAdaptorApi::MoveExAxisFun(double dDist, int nSpeed, int nMoveXYZAxisNo, double dMaxExSpeed)
{
	if (dMaxExSpeed < 0.0)
	{
		dMaxExSpeed = nSpeed;
...
int EstunAdaptorApi::XYZAxisNoToSoftAxisNo(int nMoveXYZAxisNo)
{
	//int nSoftAxisNo = -1;
...
bool EstunAdaptorApi::CheckIsReadyRun()
{
...
double EstunAdaptorApi::GetExPositionDis(int nMoveXYZAxisNo)
{
	double dCurPos = 0.0;
	T_ROBOT_COORS tRobotCoors = GetCurrentPos();
	if (1 == nMoveXYZAxisNo) {
...
E_ERROR_STATE EstunAdaptorApi::CheckRobotDone(int nDelayTime)
{
	int retValue = 0;
	while (1)
	{
...
bool EstunAdaptorApi::WorldIsRunning() {
...
void EstunAdaptorApi::WorldCheckRobotDone(int nDelayTime)
{
	CheckRobotDone();
}

void EstunAdaptorApi::HoldOn()
{
	m_pEstunRobot->Stop();
}

void EstunAdaptorApi::HoldOff()
{
}


//埃斯顿机器人一次设置大量变量 直角
bool EstunAdaptorApi::SetMultiPosVar(UINT unIndex, std::vector<T_ROBOT_COORS> vtRobotJointCoord, T_ROBOT_MOVE_SPEED tPosMove, int config[7], UINT unToolNum)
{
	if (NULL == config)
	{
		for (int i = 0; i < 7; i++)
...
bool EstunAdaptorApi::SetMultiPosVar(UINT unIndex, std::vector<T_ROBOT_COORS> vtRobotJointCoord, T_ROBOT_MOVE_SPEED tPosMove, CString Program_name, int config[7], UINT unToolNum)
{
	if (NULL == config)
	{
		for (int i = 0; i < 7; i++)
...
bool EstunAdaptorApi::SendTeachMoveData(const std::vector<T_ANGLE_PULSE>& vtMeasurePulse, const std::vector<int>& vnMeasureType, int nDownSpeed, int nTeachSpeed, int nUpSpeed, int nTrigSigTime)
{
	// 工程变量
	// mJP1 - mJP20 : 测量关节坐标
...
double EstunAdaptorApi::GetCurrentPos(int nAxisNo)
{
	double adPos[10] = { 0.0 };
...
T_ROBOT_COORS EstunAdaptorApi::GetCurrentPos_ESTUN()
{
	//使用EstunApi获取当前位置
...
T_ROBOT_COORS EstunAdaptorApi::GetCurrentPos()
{
	T_ROBOT_COORS tRobotCurCoord;
...
long EstunAdaptorApi::GetCurrentPulse(int nAxisNo)
{
	double adPos[10] = { 0.0 };
	m_pEstunRobot->GetCurAxis_Fast(adPos);
...
T_ANGLE_PULSE EstunAdaptorApi::GetCurrentPulse()
{
	...
	tRobotCurPulses.lBXPulse = adPos[6] / m_tExternalAxle[0].dPulse;
	tRobotCurPulses.lBYPulse = adPos[7] / m_tExternalAxle[1].dPulse;
	tRobotCurPulses.lBZPulse = adPos[8] / m_tExternalAxle[2].dPulse;
	return tRobotCurPulses;
}

double EstunAdaptorApi::GetPositionDis() 
{
	double adPos[10] = { 0.0 };
	m_pEstunRobot->GetCurPos_Fast(adPos);
	return adPos[6];
