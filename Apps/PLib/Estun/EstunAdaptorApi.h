...
class EstunAdaptorApi : public CRobotDriverAdaptor
{
public:
	EstunAdaptorApi(CString strUnitName, CLog* cLog, std::vector<CUnitDriver*>* ppUnitDriver);
	virtual ~EstunAdaptorApi();
...
public:
	 ...
	 * @return {*} 修改是否成功
	 */
	virtual bool EstunTrackInit(const T_ROBOT_COORS& startPos, double dTrackCurrent, double dTrackVoltage, const CString& IP = "192.168.60.63", double Speed = 420.0, const vector<int>& Cfg = vector<int>(7, 0));
	/**
	...
	virtual bool MoveToSafeHeight() override;
	...
	int WeldMoveTriangleWeaveByRobot(const std::vector<T_ROBOT_COORS>& vtWeldPathPoints, const  std::vector<int>& vnPtnType, const T_WELD_PARA& tWeldPara, double dExAxlePos, E_WELD_SEAM_TYPE eWeldSeamType, bool bIsArcOn);
	...
	bool SendWeldTriangleWeaveProgramRobot(int nWeldTrackNum, E_WELD_SEAM_TYPE eWeldSeamType);
