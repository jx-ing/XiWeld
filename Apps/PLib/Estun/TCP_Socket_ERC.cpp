...
{
	//std::unique_lock<std::mutex> lock(m_Tcpmutex); //构造自动加锁,退出作用域自动解锁
	...
	else
	{
		...
		//成功发送
		//int flag = 1;
		//if (setsockopt(Socket, IPPROTO_TCP, TCP_NODELAY, (const char*)&flag, sizeof(flag)) == SOCKET_ERROR)
		//{
		//	// 可选：记录setsockopt失败的错误
		//}
		std::ofstream RecordFile("./EstunComRecord.txt", std::ios::app); //用记事本的方式记录下来
		           										   //time_t t = time(NULL);
		SYSTEMTIME st = { 0 };
		...
		else//发送失败
		{
			...
			RecordFile << st.wSecond << "-" << st.wMilliseconds << ":" << "服务器 " << Record_IP << ":" << str <<
				" :客户端未连接，失败" << " WSAGetLastError：" << error << "\n";
