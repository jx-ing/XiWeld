...
#include "Apps/ImageProcess/FlatWeldRealTimeTrack.h"
#include "Apps/ImageProcess/PartialPointCloudProcess.h"
...
// 板厚，跳枪用
	...
	// wrz新跟踪函数接口入参
	WidgetLaserInfo* m_pLaserInfo = new WidgetLaserInfo[10];
};


struct TimeStampedPose {
	long long timestamp;	//时间戳
	T_ROBOT_COORS coord;	//当前直角
	T_ANGLE_PULSE pulse; //当前脉冲
};
...
/*	****************************函	数****************************	*/
	...
	bool CaptureTrackImage(int nCameraNo, int &nImageNo, IplImage** pImageBuff, bool bSaveImage);
	...
	bool m_bJudgeEndReturn = false;
	bool m_IsOpenRobotPoseTime;
	...
	bool DynamicCaptureForEndpoint(const std::shared_ptr<image_process::FlatWeldEndpointSearch>& pImageProcess,
		CRobotDriverAdaptor* pRobotDriver, int nCameraNo, int nTimeOut = 5000); // 点云方式：端点搜索图片采集和处理
	bool DynamicCaptureForVert(const std::shared_ptr<image_process::VertWeldSearch>& pImageProcess, 
		CRobotDriverAdaptor* pRobotDriver, int nTimeOut = 5000); // 点云方式：端点搜索图片采集和处理
	//全扫描动态采图
	...
	void RobotPoseTime(CRobotDriverAdaptor* pRobotDrive);
	static UINT RobotPoseThread(void* pParam);
	T_ROBOT_COORS InterpolateCoord(const TimeStampedPose& pose1, const TimeStampedPose& pose2, long long targetTimestamp);
	bool FindNearestPosePair(long long targetTime, TimeStampedPose& before, TimeStampedPose& after);
	bool GetInterpolatedPose(bool bIsFirst, CRobotDriverAdaptor* pRobotDrive, long long imageTimestamp, T_ROBOT_COORS& interpCoord, T_ANGLE_PULSE& interpPulse);
...
/*	****************************参	数****************************	*/
	...
	std::deque<TimeStampedPose> g_poseBuffer;
	std::mutex g_mutex;
	/*****	搜端点使用的public参数	*****/
...
public:
	...
	image_process::PartialPointCloudProcess m_cPartialPointCloudProcess;//局部点云处理类

	//跟踪图像处理
	image_process::FlatWeldRealTimeTrack* m_pFlatWeldRealTimeTrack = nullptr;
};
