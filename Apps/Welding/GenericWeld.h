...
private:
	...
	// 计算平焊自由端点测量数据
	bool CalcFreeTeachDataFlat(int nGroupNo, int nSeamNo, bool bIsFlatSeam, bool bIsStartPtn, LineOrCircularArcWeldingLine tSeam, double dNorAngle,
		double dSearchDis, double dSearchDis2, double dExAxlePos, vector<T_TEACH_DATA>& vtTeachData);
	
	// 计算平焊干涉端点测量数据（搜索）
	bool CalcInterfereTeachDataFlatBySearch(int nGroupNo, int nSeamNo, bool bIsFlatSeam, bool bIsStartPtn, LineOrCircularArcWeldingLine tSeam, double dNorAngle, double dSearchDis, double dExAxlePos, vector<T_TEACH_DATA>& vtTeachData);
