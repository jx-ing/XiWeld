...
#include "Apps/ImageProcess/TeachImage.h"
#include "Apps/ImageProcess/FlatWeldEndpointSearch.h"
#include "Apps/ImageProcess/VertWeldSearch.h"
...
void WeldAfterMeasure::LoadEquipmentParam()
{
	...
	m_nRobotInstallDir = PARA_SYSTEM(nRobotInstallDir);
...
bool WeldAfterMeasure::LoadCloudProcessResultNew(CString sFileName/* = ""*/)
{
	...
	int i = 0;
	double tmp;
	while (EOF != fscanf(pf, "%d %d %d %lf %d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %d %d %d %d %d %lf %lf %lf %d %lf %lf",
		&nIdx, &tWeld.IsArc,
...
{
	double dDirAngle = dRz;// (dRz - m_tRobotHomeCoors.dRZ) + m_dWeldNorAngleInHome;
	dDirAngle = fmod(dDirAngle, 360.0);
...
bool WeldAfterMeasure::CheckContinueMove(int nGroupNo, /*double dCarPos,*/ double dExAxisPos, double yPos) {
	...
	for (int i = 0; i < 8; i++) {
		...
		if (E_PLAT_GROOVE != eWeldSeamType) {
			tDownCoord.dX += (PARA_SYSTEM(dUpOrDownGunSafeHeight) / 1.2 * CosD(m_pRobotDriver->RzToDirAngle(tDownCoord.dRZ)));
			tDownCoord.dY += (PARA_SYSTEM(dUpOrDownGunSafeHeight) / 1.2 * SinD(m_pRobotDriver->RzToDirAngle(tDownCoord.dRZ)));
			tDownCoord.dRY = PARA_FLAT_WELD(dWeldPosRY); // 过渡点 Ry 45 安全
		}
		tDownCoord.dZ = (dMaxHeight + (PARA_SYSTEM(dUpOrDownGunSafeHeight) * 2.0) * ((double)m_nRobotInstallDir));
		...
		tBackCoord.dX += (PARA_SYSTEM(dUpOrDownGunSafeHeight) / 1.2 * CosD(m_pRobotDriver->RzToDirAngle(tBackCoord.dRZ)));
		tBackCoord.dY += (PARA_SYSTEM(dUpOrDownGunSafeHeight) / 1.2 * SinD(m_pRobotDriver->RzToDirAngle(tBackCoord.dRZ)));
		tBackCoord.dRY = PARA_FLAT_WELD(dWeldPosRY); // 过渡点 Ry 45 安全
		tBackCoord.dZ = (dMaxHeight + (PARA_SYSTEM(dUpOrDownGunSafeHeight) * 2.0) * ((double)m_nRobotInstallDir));
...
bool WeldAfterMeasure::ProcessTeachImage(int nTeachPtnNo, GROUP_STAND_DIP_NS::CImageProcess* pImgProcess, IplImage* pImg, T_ANGLE_PULSE tCapPulse, int nMeasureType, T_TEACH_RESULT &tTeachResult, E_FLIP_MODE eFlipMode)
{
	...
	{
		int PtnNum = WidgetLaserLock(pImg, LaserInfo, true, !PARA_FLAT_MEASURE(bCrossFilp), "WidgetLaserLock_HeightMeasure");
		if (PtnNum <= 0)
		{
	...
	{
		int PtnNum = WidgetLaserLock(pImg, LaserInfo, false, !PARA_FLAT_MEASURE(bCrossFilp), LogName);
		if (PtnNum <= 0)
		{
	...
	WriteLog("先测后焊中心二维点：%d %d", tKeyPoint.x, tKeyPoint.y);
	...
	T_ALGORITHM_POINT tAlgStartPtn, T_ALGORITHM_POINT tAlgEndPtn, double dNorAngle, double dZSide)
{
	...
	// 与识别结果对比
	double dRecoTeachDisErrThreshold = PARA_FLAT_MEASURE(dRecoTeachDisErrThreshold);
#ifdef SINGLE_ROBOT
...
bool WeldAfterMeasure::AdjustScanWeldSeam(int nGroupNo, int nWeldNo, std::vector<T_TEACH_DATA>& tTeachData)
{
	...
	if (vtAlgPtns.size() > 2)
	{
		tAlgStartPtn = vtAlgPtns[vtAlgPtns.size() - 3];
		tAlgEndPtn = vtAlgPtns[vtAlgPtns.size() - 2];
	}
...
bool WeldAfterMeasure::AdjustWeldSeamArc(int nGroupNo, int nWeldNo, std::vector<T_TEACH_DATA>& tTeachData)
{
	...
	for (int nPtnIdx = 0; nPtnIdx < nPtnNum; nPtnIdx++)
	{
		...
		T_ROBOT_COORS tCoord = GenerateRobotCoord(tSpaceCircleParam.tCenterPoint, 
			PARA_FLAT_WELD(dWeldPosRX), PARA_FLAT_WELD(dWeldPosRY), DirAngleToRz(dCurDirAngle));
...
{
	if (dStandWeldWrapLen > PARA_VER_WELD(dMaxWrapLength))
	{
		dStandWeldWrapLen = PARA_VER_WELD(dMaxWrapLength);
	}
	...
	double dWrapEndPostureRy = PARA_FLAT_WELD(dWeldPosRY)/*45.0*/; // 立焊包角恢复到平焊姿态
	bool bRst = false;
	...
	case E_STAND_SEAM:
		...
		if (false == PARA_VER_WELD(bAutoCalcWrapDirStand) || fabs(dHeightDiff) < 1.5)
		{
#ifdef SINGLE_ROBOT
...
{
	bool bOnlyProcessComp = PARA_FLAT_WELD(bFlatWeldContinue); //false;
		...
		((E_STAND_SEAM == eWeldSeamType) && (dWeldAngleComp < 20.0 || dWeldAngleComp > 70.0)))
	{
		...
		dWeldAngleComp = PARA_FLAT_WELD(dWeldPosRY);
	}
...
void WeldAfterMeasure::CalcCorvrAngle(bool bBelongStart, bool bBelongPtnFree, bool bOtherPtnFree, T_ROBOT_COORS& tCoord, double dSeamLen/* = 0*/)
{
	// 测量点属于起点附近还是终点附近 ？ 
	...
	if (bBelongPtnFree && (!bOtherPtnFree) && (dSeamLen < PARA_FLAT_MEASURE(dShortSeamThreshold)))
	{
		bBelongStart = !bBelongStart;
	}
	//double dChangeAngle = bBelongStart ? m_dGunLaserAngle : m_dGunCameraAngle; // 起点附近遮挡激光 终点附近遮挡相机
	double dChangeAngle = bBelongStart ? PARA_SYSTEM(dGunCameraAngle) : PARA_SYSTEM(dGunLaserAngle); // 起点附近遮挡激光 终点附近遮挡相机
	double dChangeDir = bBelongStart ? PARA_SYSTEM(dRotateToCamRxDir) : -PARA_SYSTEM(dRotateToCamRxDir); // 挡激光转向相机 挡相机转向激光
...
bool WeldAfterMeasure::TeachProcess(int nTimeOut)
{
	...
	m_pRobotDriver->m_cLog->Write("测量应该采图数量：%d", nTeachPtnTypeNum);

	// 开启回调采图
	image_process::TeachImage cTeachImage;
	cTeachImage.setShowImage(m_pColorImg);
	cTeachImage.setHardware(m_ptUnit, m_ptUnit->m_nMeasureCameraNo);
	cTeachImage.createImageBuffer();
	cTeachImage.deleteOldData();
	auto pBufferImg = cTeachImage.getSrcImage();
	cTeachImage.openCamera(E_ACQUISITION_MODE_SOURCE_HARD_0, E_CALL_BACK_MODE_SAVE_IMAGE);
	if (!cTeachImage.startAcquisition())
	{
		XUI::MesBox::PopError("示教开启回调采图失败！");
		return false;
	}
	...
	while (true)
	{
		...
		if (nCapNum <= nProIdx)
		{
			...
			{
				XUI::MesBox::PopError("等待第{0}张示教图片超时 {1} {2}", nProIdx, nCurWaitTime, nTimeOut);
				break;
			}
			continue;
		}
		...
		// 存原图
		cTeachImage.saveSrcImage(nProIdx);

		// 图像处理 根据不同处理类型 调用图像处理得到二维点
		if (!cTeachImage.initImageProcessPara(nProIdx, vnMeasurePtnType[nProIdx], m_dTeachExAxlePos, vtTeachPulse[nProIdx])
			|| !cTeachImage.processImage())
		{
			break;
		}
		m_vtTeachResult.push_back(cTeachImage.getTeachResult());

		// 显示并保存处理后图片
		cTeachImage.drawResult();
		cTeachImage.showDstImage();
		cTeachImage.saveDstImage(nProIdx);
...
bool WeldAfterMeasure::SpuriousTriggerTeach(int nGroupNo, 
	const std::vector<T_ANGLE_PULSE>& vtMeasurePulse, 
	const vector<int>& vnMeasureType, double dExAxlePos)
{
	...
	if (0 != m_ptUnit->MoveExAxisFun(dExAxlePos, PARA_FLAT_MEASURE(dExAxisPosMoveSpeed), nAxisNo))
	{
		return false;
	}
	m_ptUnit->WorldCheckRobotDone();
	...
	{
		XUI::MesBox::PopError("自动示教:外部轴未运动到指定位置");
		return false;
	}

	// 机器人前往第一个示教点
	T_ROBOT_MOVE_SPEED tPulseMove = m_pRobotDriver->m_tPulseLowSpeed;
	tPulseMove.dSpeed = PARA_FLAT_MEASURE(dExAxisPosMoveSpeed);
	m_pRobotDriver->MoveByJob(vtMeasurePulse[0], m_pRobotDriver->m_tPulseLowSpeed,
		m_pRobotDriver->m_nExternalAxleType, "MOVJ");

	// 下枪过程中初始化
	cTeachImage.deleteOldData();
	cTeachImage.openCamera(E_ACQUISITION_MODE_SOURCE_SOFTWARE, E_CALL_BACK_MODE_OFF);
	if (!cTeachImage.startAcquisition())
	{
		XUI::MesBox::PopError("示教开启采图失败！");
		return false;
	}
	...
	bool bRtn = true;
	int nProIndex = 0;
	for (int n = 0; n < vtMeasurePulse.size(); n++)
	{
		// 机器人前往下一个示教点
		if (n > 0)
		{
			m_pRobotDriver->MoveByJob(vtMeasurePulse[n], m_pRobotDriver->m_tTeachSpeed,
				m_pRobotDriver->m_nExternalAxleType, "MOVJ");
			m_ptUnit->WorldCheckRobotDone();
			if (!m_pRobotDriver->ComparePulse(vtMeasurePulse[n], 100))
			{
				XUI::MesBox::PopError("机器人未运行到位");
				bRtn = false;
				break;
		...
		// 采图
		if (!cTeachImage.captureImage(3))
		{
			XUI::MesBox::PopError("示教软触发采图失败");
			bRtn = false;
			break;
		}

		// 存原图
		cTeachImage.saveSrcImage(nProIndex);

		// 图像处理 根据不同处理类型 调用图像处理得到二维点
		if(!cTeachImage.initImageProcessPara(nProIndex, vnMeasureType[n], m_dTeachExAxlePos, vtMeasurePulse[n])
			|| !cTeachImage.processImage())
		{
			bRtn = false;
			break;
		...
		cTeachImage.drawResult();
		cTeachImage.showDstImage();
		cTeachImage.saveDstImage(nProIndex);
...
bool WeldAfterMeasure::DoTeach(int nGroupNo, const std::vector<T_ANGLE_PULSE>& vtSrcMeasurePulse, const vector<int>& vnSrcMeasureType, double dExAxlePos, int nLayerNo/* = 0*/)
{
	...
	{
		if (0 != m_ptUnit->MoveExAxisFun(dExAxlePos_y, PARA_FLAT_MEASURE(dExAxisPosMoveSpeed), 2))return false;
		m_ptUnit->WorldCheckRobotDone();
		// 需要更改获取轴数据，临时更改
	...
	if (vtSearchPulse.size() > 0)
	{
		...
		for (int i = vtSearchPulse.size(); i > 0; i -= nSearchPointNum)
		{
			...
			{
				if (0 != m_ptUnit->MoveExAxisFun(dExPos, PARA_FLAT_MEASURE(dExAxisPosMoveSpeed), nAxisNo))
				{
					return false;
				}
				m_ptUnit->WorldCheckRobotDone();
			...
			if (-1 != m_ptUnit->m_nMeasureAxisNo_up)
			{
				...
				{
					if (0 != m_ptUnit->MoveExAxisFun(dExPos_y, PARA_FLAT_MEASURE(dExAxisPosMoveSpeed), nUpAxisNo))
					{
						return false;
					}
					m_ptUnit->WorldCheckRobotDone();
	...
	const vector<int>& vnPtnType, const T_WELD_PARA& tWeldPara)
{
	...
	if (E_PLAT_GROOVE != eWeldSeamType)
	{
		if (E_STAND_SEAM == eWeldSeamType&&m_bIsSlope==true)
		{
			...
			tDownCoord.dX += (PARA_SYSTEM(dUpOrDownGunSafeHeight) / 1.2 * CosD(RzToDirAngle(tDownCoord.dRZ)));
			tDownCoord.dY += (PARA_SYSTEM(dUpOrDownGunSafeHeight) / 1.2 * SinD(RzToDirAngle(tDownCoord.dRZ)));
			tDownCoord.dRY = 45.0; // 过渡点 Ry 45 安全
		}
	}
	tDownCoord.dZ = (dMaxHeight + PARA_SYSTEM(dUpOrDownGunSafeHeight) * ((double)m_nRobotInstallDir));
	...
	if (E_PLAT_GROOVE != eWeldSeamType)
	{
		tBackCoord.dX += (PARA_SYSTEM(dUpOrDownGunSafeHeight) / 1.2 * CosD(RzToDirAngle(tBackCoord.dRZ)));
		tBackCoord.dY += (PARA_SYSTEM(dUpOrDownGunSafeHeight) / 1.2 * SinD(RzToDirAngle(tBackCoord.dRZ)));
		tBackCoord.dRY = 45.0; // 过渡点 Ry 45 安全
	}
	tBackCoord.dZ = (dMaxHeight + PARA_SYSTEM(dUpOrDownGunSafeHeight) * ((double)m_nRobotInstallDir));
...
//#endif // !SINGLE_ROBOT
	...
	if (false == m_bIsLocalDebug) // 焊接
	{
		...
		pRobotDriver->WorldCheckRobotDone();
		T_ROBOT_COORS tCompareCoord = vtWeldPathPoints.back();
		T_ROBOT_COORS tCompareCoord2 = vtWeldPathPoints.back();
		auto tCurCoord = m_pRobotDriver->GetCurrentPos();
		if(tCurCoord.dX == 0 && tCurCoord.dY == 0 && tCurCoord.dZ == 0)
		{
			Sleep(10);
		...
		{
			tCompareCoord = vtWeldPathPoints[vtWeldPathPoints.size() - 2];
		}
		else
		{
			tCompareCoord = m_ptUnit->m_cWeldTrack.back();
		}
		if (!m_pRobotDriver->CompareCoords(tCompareCoord, tCurCoord)
			&& !m_pRobotDriver->CompareCoords(tCompareCoord2, tCurCoord))
		{
			XUI::MesBox::PopError("当前位置{},{},{},{},{},{},{},{},{},未到达实际终点{},{},{},{},{},{},{},{},{}",
				tCurCoord.dX, tCurCoord.dY, tCurCoord.dZ,
				tCurCoord.dRX, tCurCoord.dRY, tCurCoord.dRZ,
				tCurCoord.dBX, tCurCoord.dBY, tCurCoord.dBZ,
				tCompareCoord.dX, tCompareCoord.dY, tCompareCoord.dZ, 
				tCompareCoord.dRX, tCompareCoord.dRY, tCompareCoord.dRZ, 
				tCompareCoord.dBX, tCompareCoord.dBY, tCompareCoord.dBZ);
			return false;
		}
	}
	WriteLog("焊接完成");
	...
	vtRobotMoveInfo.clear();
	tRobotMoveInfo = m_pRobotDriver->PVarToRobotMoveInfo(0, tBackCoord, m_pRobotDriver->m_tBackGunSpeed, MOVL);
	vtRobotMoveInfo.push_back(tRobotMoveInfo);
...
bool WeldAfterMeasure::ScanEndpoint(int nGroupNo, int nCameraNo, vector<T_ANGLE_PULSE> vtPulse, vector<int> vnType, vector<T_TEACH_RESULT>& vtTeachResult)
{
	...
	std::shared_ptr<image_process::GenericImage> pImageProcess;
	if(bScanMode)
		pImageProcess = std::make_shared<image_process::VertWeldSearch>();
	else
		pImageProcess = std::make_shared<image_process::FlatWeldEndpointSearch>();
	...
	m_pRobotDriver->CallJob("CONTIMOVANY");
	pImageProcess->setShowImage(m_pColorImg);
	pImageProcess->setHardware(m_ptUnit, m_ptUnit->m_nMeasureCameraNo);
	pImageProcess->createImageBuffer();
	pImageProcess->deleteOldData();
	pImageProcess->openCamera(E_ACQUISITION_MODE_SOURCE_SOFTWARE, E_CALL_BACK_MODE_OFF);
	if (!pImageProcess->startAcquisition())
	{
		XUI::MesBox::PopError("扫描开启采图失败！");
		return false;
	}
	m_ptUnit->WorldCheckRobotDone();
	...
	{
		double dScanSpeed = PARA_FLAT_MEASURE(dLineScanSpeed); 
		double dScanSpeedHeight = dScanSpeed;
		if (bScanMode)
		{
			dScanSpeed = PARA_VER_MEASURE(dLineScanSpeed);
			dScanSpeedHeight = PARA_VER_MEASURE(dLineScanSpeedHeight);
		}
		if (ROBOT_BRAND_YASKAWA == m_pRobotDriver->m_eRobotBrand)
		{
			dScanSpeed = dScanSpeed / 6;// 安川机器人MOVL使用整形变量作为速度需要除以6（需要改到底层）
		}
		vtRobotMoveInfo.clear();
		T_ROBOT_MOVE_SPEED tScanSpeed(dScanSpeed, 100, 100);
		T_ROBOT_MOVE_SPEED tScanSpeedHeight(dScanSpeedHeight, 100, 100);
		...
		//double dMinDis = 1e6;
		//int nScanWeldNo = -1;
		//CvPoint3D64f tCoord1{ vtCoord[1].dX + vtCoord[1].dBX, vtCoord[1].dY + vtCoord[1].dBY, vtCoord[1].dZ + vtCoord[1].dBZ };
		//CvPoint3D64f tCoord2{ vtCoord[2].dX + vtCoord[2].dBX, vtCoord[2].dY + vtCoord[2].dBY, vtCoord[2].dZ + vtCoord[2].dBZ };
		//for (size_t i = 0; i < m_vvtWeldLineInfoGroup[nGroupNo].size(); i++)
		//{
		//	if (!IsStandWeldSeam(m_vvtWeldLineInfoGroup[nGroupNo][i].tWeldLine.StartNormalVector))
		//	{
		//		continue;
		//	}
		//	auto& tStart = m_vvtWeldLineInfoGroup[nGroupNo][i].tWeldLine.StartPoint;
		//	double dDis = TwoPointDis2D(tStart, tCoord1) + TwoPointDis2D(tStart, tCoord2);
		//	if (dDis < dMinDis)
		//	{
		//		dMinDis = dDis;
		//		nScanWeldNo = i;
		...
		{
			//去除外部轴的立焊起点
			auto tSeamStart = m_vvtWeldLineInfoGroup[nGroupNo][0].tWeldLine.StartPoint;
			tSeamStart.x -= vtCoord[1].dBX;
			tSeamStart.y -= vtCoord[1].dBY;
			tSeamStart.z -= vtCoord[1].dBZ;
			//去除外部轴的立焊终点
			auto tSeamEnd = m_vvtWeldLineInfoGroup[nGroupNo][0].tWeldLine.EndPoint;
			tSeamEnd.x -= vtCoord[1].dBX;
			tSeamEnd.y -= vtCoord[1].dBY;
			tSeamEnd.z -= vtCoord[1].dBZ;
			//起点扫描轨迹
			T_ROBOT_COORS tScanStartCoord{
				tSeamStart.x,
				tSeamStart.y,
				(vtCoord[1].dZ + vtCoord[2].dZ) / 2.0,
				fabs(vtCoord[1].dRX - vtCoord[2].dRX) > 180.0 ?
					xi::math::makesureInCycle<-180, 180>((vtCoord[1].dRX + vtCoord[2].dRX) / 2.0 + 180.0) :
					xi::math::makesureInCycle<-180, 180>((vtCoord[1].dRX + vtCoord[2].dRX) / 2.0),
				fabs(vtCoord[1].dRY - vtCoord[2].dRY) > 180.0 ?
					xi::math::makesureInCycle<-180, 180>((vtCoord[1].dRY + vtCoord[2].dRY) / 2.0 + 180.0) :
					xi::math::makesureInCycle<-180, 180>((vtCoord[1].dRY + vtCoord[2].dRY) / 2.0),
				fabs(vtCoord[1].dRZ - vtCoord[2].dRZ) > 180.0 ?
					xi::math::makesureInCycle<-180, 180>((vtCoord[1].dRZ + vtCoord[2].dRZ) / 2.0 + 180.0) :
					xi::math::makesureInCycle<-180, 180>((vtCoord[1].dRZ + vtCoord[2].dRZ) / 2.0),
				vtCoord[1].dBX,
				vtCoord[1].dBY,
				vtCoord[1].dBZ
			};
			//终点扫描轨迹
			T_ROBOT_COORS tScanEndCoord{
				tSeamEnd.x,
				tSeamEnd.y,
				tSeamEnd.z,
				fabs(vtCoord[1].dRX - vtCoord[2].dRX) > 180.0 ?
			...
			WriteLog("立焊起点扫描：%.3lf %.3lf %.3lf %.3lf %.3lf %.3lf %.3lf %.3lf %.3lf",
				tScanStartCoord.dX, tScanStartCoord.dY, tScanStartCoord.dZ,
				tScanStartCoord.dRX, tScanStartCoord.dRY, tScanStartCoord.dRZ,
				tScanStartCoord.dBX, tScanStartCoord.dBY, tScanStartCoord.dBZ);
			WriteLog("立焊终点扫描：%.3lf %.3lf %.3lf %.3lf %.3lf %.3lf %.3lf %.3lf %.3lf",
				tScanEndCoord.dX, tScanEndCoord.dY, tScanEndCoord.dZ,
				tScanEndCoord.dRX, tScanEndCoord.dRY, tScanEndCoord.dRZ,
				tScanEndCoord.dBX, tScanEndCoord.dBY, tScanEndCoord.dBZ);
			// 焊枪工具 转 跟踪相机工具
			if (!m_pRobotDriver->MoveToolByWeldGun(tScanStartCoord, m_pRobotDriver->m_tTools.tGunTool,
				tScanStartCoord, m_pRobotDriver->m_tTools.tCameraTool, tScanStartCoord))
			{
				XiMessageBox("立焊起点测量点 转换失败！");
				return false;
			}
			if (!m_pRobotDriver->MoveToolByWeldGun(tScanEndCoord, m_pRobotDriver->m_tTools.tGunTool,
				tScanEndCoord, m_pRobotDriver->m_tTools.tCameraTool, tScanEndCoord))
			{
				XiMessageBox("立焊起点测量点 转换失败！");
				return false;
			}

			if (fabs(dWeldEndZ - dWeldStartZ) > 120.0) // 扫描起点终点
			{
				...
				tRobotMoveInfo = m_pRobotDriver->PVarToRobotMoveInfo(0, tScanCoord, tScanSpeedHeight, MOVL);
				vtRobotMoveInfo.push_back(tRobotMoveInfo);

				tScanCoord = tScanStartCoord;
				tScanCoord.dZ = dScanStartZ;
				...
				tScanCoord.dZ = dScanStartZ;
				tRobotMoveInfo = m_pRobotDriver->PVarToRobotMoveInfo(2, tScanCoord, tScanSpeed, MOVL);
				vtRobotMoveInfo.push_back(tRobotMoveInfo);

				tScanCoord = vtCoord[1];
				tScanCoord.dZ = dScanEndZ;
				tRobotMoveInfo = m_pRobotDriver->PVarToRobotMoveInfo(3, tScanCoord, tScanSpeedHeight, MOVL);
				vtRobotMoveInfo.push_back(tRobotMoveInfo);

				tScanCoord = tScanEndCoord;
				tScanCoord.dZ = dScanEndZ;
				tRobotMoveInfo = m_pRobotDriver->PVarToRobotMoveInfo(4, tScanCoord, tScanSpeed, MOVL);
				vtRobotMoveInfo.push_back(tRobotMoveInfo);

				tScanCoord = vtCoord[2];
				tScanCoord.dZ = dScanEndZ;
				tRobotMoveInfo = m_pRobotDriver->PVarToRobotMoveInfo(5, tScanCoord, tScanSpeed, MOVL);
				vtRobotMoveInfo.push_back(tRobotMoveInfo);
			}
			else//扫描起点
			{
				...
				tRobotMoveInfo = m_pRobotDriver->PVarToRobotMoveInfo(0, vtCoord[1], tScanSpeed, MOVL);
				vtRobotMoveInfo.push_back(tRobotMoveInfo);
				tRobotMoveInfo = m_pRobotDriver->PVarToRobotMoveInfo(1, tScanStartCoord, tScanSpeed, MOVL);
				vtRobotMoveInfo.push_back(tRobotMoveInfo);
				tRobotMoveInfo = m_pRobotDriver->PVarToRobotMoveInfo(2, vtCoord[2], tScanSpeed, MOVL);
				vtRobotMoveInfo.push_back(tRobotMoveInfo);
			}
		}
		// 高立焊扫描起点终点添加 End
		...
		if (bScanMode)//立焊扫描
		{
			CHECK_FALSE_RETURN(m_pScanInit->DynamicCaptureForVert(
				std::dynamic_pointer_cast<image_process::VertWeldSearch>(pImageProcess),
				m_pRobotDriver));
		}
		else//平焊搜端点
		{
			//CHECK_FALSE_RETURN(m_pScanInit->lockLaser(m_pRobotDriver, m_ptUnit->m_nMeasureCameraNo));
			CHECK_FALSE_RETURN(m_pScanInit->DynamicCaptureForEndpoint(
				std::dynamic_pointer_cast<image_process::FlatWeldEndpointSearch>(pImageProcess),
				m_pRobotDriver, m_ptUnit->m_nMeasureCameraNo));
		}
	}
	else // 单点运动测量
	{
	...
	if (0)
	{
		...
		m_pRobotDriver->SetMoveValue(vtRobotMoveInfo, false);
		m_pRobotDriver->CallJob("CONTIMOVANY");
		pImageProcess->closeCamera(); // 收枪时关闭相机和激光
		m_ptUnit->WorldCheckRobotDone();
...
bool WeldAfterMeasure::GetScanProcResult(int nGroupNo, bool bIsScanMode, bool bSinglePointMeasure, vector<T_ROBOT_COORS> vtCoord, vector<T_TEACH_RESULT>& vtTeachResult)
{
	...
	else
	{
		...
		if (nWeldInfoNum <= 0)
		{
			nWeldInfoNum = GetRiserEndPoint(vtCloudPt.data(), vtCloudPt.size(), tWeldInfo, tCameraNorm, tPlaneHNorm,
				tRefPtn, 2, "GetRiserEndPoint_Stand-nb");
		}
...
void WeldAfterMeasure::GetAdjoinSeam(LineOrCircularArcWeldingLine tSeam, vector<LineOrCircularArcWeldingLine>& vtSeamGroup, LineOrCircularArcWeldingLine** ptAdjoinSeamS, LineOrCircularArcWeldingLine** ptAdjoinSeamE)
{
	...
	*ptAdjoinSeamE = NULL;
	...
	const double dEndPointDisThreshold = 3.0; // 常量
	...
	if (bIsStandSeam)
	{
		for (int nOtherIdx = 0; nOtherIdx < vtSeamGroup.size(); nOtherIdx++) // 起点与立焊起点相邻的平焊缝
		{
			...
			if (dEndPointDisThreshold > TwoPointDis3D(tCurSeam.StartPoint, tOtherSeam.StartPoint))
			{
		...
		for (int nOtherIdx = 0; nOtherIdx < vtSeamGroup.size(); nOtherIdx++) // 终点与立焊起点相邻的平焊缝
		{
			...
			if (dEndPointDisThreshold > TwoPointDis3D(tCurSeam.StartPoint, tOtherSeam.EndPoint))
			{
	...
	else
	{
		...
		for (int nOtherIdx = 0; nOtherIdx < vtSeamGroup.size(); nOtherIdx++) // 平焊终点相邻平焊缝
		{
			...
			if (dEndPointDisThreshold > TwoPointDis3D(tCurSeam.EndPoint, tOtherSeam.StartPoint))
			{
				*ptAdjoinSeamE = &vtSeamGroup[nOtherIdx];
				break;
			}
...
bool WeldAfterMeasure::GetWorkPieceName(E_WORKPIECE_TYPE eWorkPieceType, CString& sWorkPieceName, CString& sWorkPieceTypeName)
{
	...
	case E_SCAN_WELD_LINE: // 扫描测量
		...
		break;
	case E_FLANGE_GENERIC: // 翼板通用
		sWorkPieceTypeName.Format("E_FLANGE_GENERIC");
		sWorkPieceName.Format("翼板通用");
		break;
	default:
		bRst = false;
...
{
	bool bFlatWeldContinue = PARA_FLAT_WELD(bFlatWeldContinue);
	...
	for (int i = 0; i < vnWeldOrder.size(); i++)
	{
		...
		double dChangeDisS = PARA_FLAT_WELD(dChangeDisOfStartPointRZ);	// 变姿态距离
		double dChangeDisE = PARA_FLAT_WELD(dChangeDisOfEndPointRZ);	// 变姿态距离
		double dChangeAngleS = PARA_FLAT_WELD(dChangeAngleOfStartPointRZ); // 变姿态角度
		double dChangeAngleE = PARA_FLAT_WELD(dChangeAngleOfEndPointRZ); // 变姿态角度
		double dPtnInterval = 2.0;  // 轨迹点间隔
		// 获取焊缝类型 生成焊接轨迹
		eWeldSeamType = GetWeldSeamType(tWeldSeam);

		double dPostureRx = E_FLAT_SEAM == eWeldSeamType ? PARA_FLAT_WELD(dWeldPosRX) : PARA_VER_WELD(dWeldPosRX);
		double dPostureRy = E_FLAT_SEAM == eWeldSeamType ? PARA_FLAT_WELD(dWeldPosRY) : PARA_VER_WELD(dWeldPosRY);
		...
		double dStepChangeAngleS = (double)m_nRobotInstallDir * dChangeAngleS / (double)nChangePtnNumS; // 相邻点姿态变化角度
		double dStepChangeAngleE = (double)m_nRobotInstallDir * dChangeAngleE / (double)nChangePtnNumE; // 相邻点姿态变化角度
		...
		if (E_STAND_SEAM == eWeldSeamType)
		{
			...
			double dDstRy = PARA_FLAT_WELD(dWeldPosRY);
		...
		if (false == CalcWrapTrack(m_vvtWeldLineInfoGroup[nGroupNo][nWeldNo], eWeldSeamType, 
			nWeldAngleSize * PARA_VER_WELD(dStandWeldWrapLenRatio),
			vtWeldPara[0].nStandWeldDir, vtWeldCoord, vnPointType))
		{
			XiMessageBox("包角轨迹计算失败");
			return false;
		}
		double outOff = m_mdFlatHorComp[405];
		double inOff = m_mdFlatHeightComp[405];
		// 添加平焊自由端及过水孔处包角（平焊包角）
		if (E_FLAT_SEAM == eWeldSeamType && (TRUE == *m_bNeedWrap) &&
			m_vvtWeldLineInfoGroup[nGroupNo][nWeldNo].tAtrribute.nStartWrapType == 7 ||
			m_vvtWeldLineInfoGroup[nGroupNo][nWeldNo].tAtrribute.nEndWrapType == 7) // 平焊 选择包角才包角
		{
			...
			*/
			...
			if (/*m_vvtWeldLineInfoGroup[nGroupNo][nWeldNo].tAtrribute.nEndWrapType == 0
				...
				|| */m_vvtWeldLineInfoGroup[nGroupNo][nWeldNo].tAtrribute.nEndWrapType == 7)
			{
				/*
					自由 有过孔 包
					自由 无过孔 包
				...
				*/
				...
				if ((!tWeldSeam.EndPointType) || (dWeldHoleDisE > 30.0 && tWeldSeam.EndPointType))
				{
					...
					tWrapPoint = RobotCoordPosOffset(tCoor, tDirCoor, tCoor, outOff);
					if (m_vvtWeldLineInfoGroup[nGroupNo][nWeldNo].tAtrribute.nEndWrapType == 0 ||
						m_vvtWeldLineInfoGroup[nGroupNo][nWeldNo].tAtrribute.nEndWrapType == 1 || m_vvtWeldLineInfoGroup[nGroupNo][nWeldNo].tAtrribute.nEndWrapType == 7)
					{
						vtWeldCoord.push_back(tWrapPoint);
						vnPointType.push_back(E_WELD_WRAP);
					}
					tWrapPoint.dX -= inOff * CosD(tWrapPoint.dRZ);
					tWrapPoint.dY -= inOff * SinD(tWrapPoint.dRZ);
					if (!tWeldSeam.EndPointType) {	//自由终点
						...
						*/
						if (m_vvtWeldLineInfoGroup[nGroupNo][nWeldNo].tAtrribute.nEndWrapType == 0 ||
							m_vvtWeldLineInfoGroup[nGroupNo][nWeldNo].tAtrribute.nEndWrapType == 1 || m_vvtWeldLineInfoGroup[nGroupNo][nWeldNo].tAtrribute.nEndWrapType == 7)
						{
							tWrapPoint.dRZ += PARA_FLAT_WELD(dWrapOffsetRZ) * m_nRobotInstallDir;
							vtWeldCoord.push_back(tWrapPoint);
							vnPointType.push_back(E_WELD_WRAP);
						}
					}
			...
			*/
				...
				||*/		m_vvtWeldLineInfoGroup[nGroupNo][nWeldNo].tAtrribute.nStartWrapType == 7)
			{
				...
				if ((!tWeldSeam.StartPointType) || (dWeldHoleDisS > 30.0 && tWeldSeam.StartPointType))
				{
					...
					tWrapPoint = RobotCoordPosOffset(tCoor, tDirCoor, tCoor, outOff);
					if (m_vvtWeldLineInfoGroup[nGroupNo][nWeldNo].tAtrribute.nStartWrapType == 0 ||
						m_vvtWeldLineInfoGroup[nGroupNo][nWeldNo].tAtrribute.nStartWrapType == 2 || m_vvtWeldLineInfoGroup[nGroupNo][nWeldNo].tAtrribute.nStartWrapType == 7)
					{
		...
		if (((E_FLAT_SEAM == eWeldSeamType) && (0 == vtWeldSeam[nWeldNo].StartPointType) && (1 == vtWeldSeam[nWeldNo].EndPointType)) ||  // 黄埔临时
...
bool WAM::WeldAfterMeasure::CalcScanTrack(LineOrCircularArcWeldingLine SeamData, std::vector<T_ROBOT_COORS>& vtMeasureCoord, vector<T_ANGLE_PULSE>& vtMeasurePulse, double& dExAxlePos)
{
	...
	tmpCoor.dRX = PARA_FLAT_MEASURE(dMeasurePosRX);
	tmpCoor.dBY = PARA_FLAT_MEASURE(dMeasurePosRY);
	//起点法相转换侧面法相
...
bool WeldAfterMeasure::SplineFiltering(double dExAxisPos, vector<XI_POINT> vtWeldTrack, vector<T_ROBOT_COORS>& vtWeldTrackRobot)
{
	...
	for (int index = 0; index < vtWeldTrack.size(); index++)
	{
		...
		for (int nSize = 0; nSize < CoutTrackPoint.size(); nSize++)
		{
			...
			T_ROBOT_COORS tRobotPoint(
				...
				PARA_FLAT_WELD(dWeldPosRX), PARA_FLAT_WELD(dWeldPosRY), m_pRobotDriver->DirAngleToRz(dTrackVerDegree), 0.0, 0.0, 0.0);
	...
	for (int n = 0; n < tTrackPtnArray.arraySize_; n++)
	{
		T_ROBOT_COORS tRobotPoint(
			...
			PARA_FLAT_WELD(dWeldPosRX), PARA_FLAT_WELD(dWeldPosRY), vtWeldTrackRobot.back().dRZ, 0.0, 0.0, 0.0);
		vtWeldTrackRobot.push_back(tRobotPoint);
	}
...
UINT WeldAfterMeasure::ThreadMeasure(void* pParam)
{
	try
	{
		WeldAfterMeasure* pObj = (WeldAfterMeasure*)pParam;
