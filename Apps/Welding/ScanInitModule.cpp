...
{
	UnInit();
	if (m_pFlatWeldRealTimeTrack)
	{
		delete m_pFlatWeldRealTimeTrack;
		m_pFlatWeldRealTimeTrack = nullptr;
	}
}
...
{
	auto dWeldPosRX = PARA_FLAT_WELD(dWeldPosRX);
	auto dWeldPosRY = PARA_FLAT_WELD(dWeldPosRY);
	vtCoutPoints.clear();
	...
	for (int index = 0; index < vtCinPoints.size(); index++)
	{
		...
		for (int nSize = 0; nSize < CoutTrackPoint.size(); nSize++)
		{
			...
			T_ROBOT_COORS tRobotPoint(CoutTrackPoint[nSize].pnt_.x_, CoutTrackPoint[nSize].pnt_.y_, CoutTrackPoint[nSize].pnt_.z_,
				dWeldPosRX, dWeldPosRY, m_pRobotDriver->DirAngleToRz(dTrackVerDegree),0.0,0.0,0.0);
...
{ 
	...
	FlipImage(m_pTraceLockImg, m_ptUnit->GetCameraParam(m_ptUnit->m_nTrackCameraNo).eFlipMode);
	
	CvPoint LeftPtn, RightPtn;// , cpBesideKeyPoint;
	bool assembleFilp = false;
	bool crossFilp = false;
	int nRst = 0;

	// ljx 七轴旧版程序暂时仅支持单层单道的跟踪
	//if (m_pTraceModel->m_nLayerNo > 0)
	//{
	//	pRobotDriver->m_cLog->Write("起弧前锁定WidgetLaserLock_MultiLayer");
	//	nRst = WidgetLaserLock(m_pTraceLockImg, m_pTraceModel->m_pLaserInfo, assembleFilp, crossFilp, "WidgetLaserLock_MultiLayer");
	//}
	//else
	//{
		pRobotDriver->m_cLog->Write("起弧前锁定WidgetLaserLock");
		nRst = WidgetLaserLock(m_pTraceLockImg, m_pTraceModel->m_pLaserInfo, assembleFilp, crossFilp, "WidgetLaserLock");
	//}

	if (nRst > 0)
	{
		cpKeyPoint = m_pTraceModel->m_pLaserInfo->crossPoint;
		LeftPtn = m_pTraceModel->m_pLaserInfo->verticalPlatePnt;
		RightPtn = m_pTraceModel->m_pLaserInfo->bottomPlatePnt;
	}
	else
	{
		CTime cTime;
		...
		XiMessageBoxOk("跟踪激光斜率锁定失败！(错误代码：找不到关键点)");
		return false;
	}
#if 0
...
#endif // 0
	/*ResumeImagePoint2D(m_pTraceLockImg->width, m_pTraceLockImg->height,cpKeyPoint, tCameraParam.eFlipMode);
	ResumeImagePoint2D(m_pTraceLockImg->width, m_pTraceLockImg->height, LeftPtn, tCameraParam.eFlipMode);
	ResumeImagePoint2D(m_pTraceLockImg->width, m_pTraceLockImg->height, RightPtn, tCameraParam.eFlipMode);*/
	...
	if (!bFlag || nRst < 0) XiMessageBoxOk("搜索激光斜率锁定失败！(错误代码：找不到关键点)");
	return bFlag;
}
...
//
//	}
//	m_cPartialPointCloudProcess.init(_T(".\\WeldData\\RobotA\\RealTimeTrack\\"));
...
void CScanInitModule::RealTimeTrackingNew(CRobotDriverAdaptor* pRobotCtrl)
{
	...
	int nJudgeEndMethod		= 3;	// 1.已知终点 2.用理论长度 3.焊接过程中判断
	int nImageNo			= -1;	// 图片序号
	int nMaxImgProFailNum	= PARA_FLAT_TRACK(nMaxImgProFailNum);	// 最大连续图像处理失败次数
	int nMaxAdjustZErrNum	= PARA_FLAT_TRACK(nMaxAdjustZErrNum);	// 最大连续Z值变化过大次数
	int nMaxFilterFailNum	= PARA_FLAT_TRACK(nMaxFilterFailNum);	// 最大连续滤波失败次数
	int nMaxErrStopNum		= PARA_FLAT_TRACK(nMaxErrStopNum);	// 最大连续检测停止次数
	int nImgProFailNum		= 0;	// 连续图像处理失败次数
	int nAdjustZErrNum		= 0;	// 连续Z值变化过大次数
	int nFilterFailNum		= 0;	// 连续滤波失败次数
	int nErrStopNum			= 0;	// 连续检测停止次数
	double dMinJoinPtnDis	= PARA_FLAT_TRACK(dMinJoinPtnDis);	// 采图测量点最小相邻距离
	...
	// 路径 和 字符串 初始化
	CString strPath = m_pFlatWeldRealTimeTrack->getDataFolder();
	...
	while (FALSE == m_pTraceModel->m_bCameraFindWeldEnd)//判断找到结尾后不再采点0
	{
		...
		CHECK_BOOL_BREAK(CaptureTrackImage(m_ptUnit->m_nTrackCameraNo, nImageNo, &pImageBuff, bSaveImage));
		if (TRUE == m_pTraceModel->m_bCameraFindWeldEnd) break;
		pRobotCtrl->m_cLog->Write("[图片采集]");

		// 4、图像处理 处理失败结束本次循环 连续多次停止跟踪 正常复位累计计数
		CHECK_BOOL_CONTINUE(TrackProcess(pRobotCtrl, nImageNo, &pImageBuff, tCapRobotCurCoord, 
			tCapRobotCurPulse, tWeldLineRobotCoord, tWeldLineWorldCoord, nImgProFailNum, 
			nMaxImgProFailNum, pfRawData));
		if (TRUE == m_pTraceModel->m_bCameraFindWeldEnd) break;
...
void CScanInitModule::ReadTrackCapCoord(CRobotDriverAdaptor* pRobotCtrl, T_ROBOT_COORS& tRobotCoord, T_ANGLE_PULSE& tRobotPulse, T_ROBOT_COORS& tWorldCoord, int nImageNo)
{
	...
	else
	{
		...
		tRobotCoord = pRobotCtrl->GetCurrentPos_ESTUN();
		if (tRobotCoord.dX == 0 && tRobotCoord.dY == 0 && tRobotCoord.dZ == 0 && tRobotCoord.dRY == 0)
		{
			tRobotCoord = pRobotCtrl->GetCurrentPos();
		}
		pRobotCtrl->RobotInverseKinematics(tRobotCoord, pRobotCtrl->m_tTools.tGunTool, vt);
...
bool CScanInitModule::CaptureTrackImage(int nCameraNo, int &nImageNo, IplImage** ppImageBuff, bool bSaveImage)
{
	bool bSuccess = true;
	...
	if (GetLocalDebugMark())
	{
		m_pFlatWeldRealTimeTrack->loadSrcImage();
	...
	{
		if (!m_pFlatWeldRealTimeTrack->captureImage(1))
		{
			return false;
		}
		m_pFlatWeldRealTimeTrack->saveSrcImage(nImageNo);
		...
		//if (bSaveImage)
		//	SaveImage(*ppImageBuff, "%s", sImageFileName); // 存图
	}
	return bSuccess;
}
...
bool CScanInitModule::TrackProcess(CRobotDriverAdaptor* pRobotCtrl, int nImageNo, IplImage** pImageBuff, T_ROBOT_COORS tRobotCoord, T_ANGLE_PULSE tRobotPulse,
	T_ROBOT_COORS& tWeldLineRobotCoord, T_ROBOT_COORS& tWeldLineWorldCoord, int &nFailNum, int nMaxFailNum, FILE *pfFile)
{
	auto dPointCloudLengthForFindEndpnt = PARA_FLAT_MEASURE(dPointCloudLengthForFindEndpnt);
	long long lTime = XI_clock();
	CString strRobot = pRobotCtrl->m_strRobotName;

	CHECK_FALSE_RETURN(m_pFlatWeldRealTimeTrack->initImageProcessPara(
		nImageNo, tRobotPulse, tRobotCoord));
	if (!m_pFlatWeldRealTimeTrack->processImage())
	{
		m_pFlatWeldRealTimeTrack->showSrcImage();
		nFailNum++;
		if (nFailNum > nMaxFailNum)
		{
			pRobotCtrl->m_cLog->Write("[图像处理]:%s 连续%d次找点数值出错，暂停焊接", strRobot, nMaxFailNum);
			CheckMachineEmg(pRobotCtrl);
			XUI::MesBox::PopError("[图像处理]:{0}号机器人 连续{1}多次找点数值出错，暂停焊接", pRobotCtrl->m_nRobotNo, nMaxFailNum);
			return false;
		}
	}
	else
	{
		nFailNum = 0;
		m_pFlatWeldRealTimeTrack->getKeyPoint(tWeldLineRobotCoord);//更新交点坐标
		m_pFlatWeldRealTimeTrack->drawResult();
		m_pFlatWeldRealTimeTrack->showDstImage();
	}
	...
	m_MutexTrackPointCloud.lock();
	int nSaveNum = 0;
	auto& points = m_pFlatWeldRealTimeTrack->getProcessResult();
	for (auto& point : points)
	{
		m_pTraceModel->ltPointCloud.push_back(point);
		//fprintf(pfPointCloud, "图号:%d 点号%d %11.3lf %11.3lf %11.3lf\n", nImageNo, nSaveNum, point.x, point.y, point.z);
		nSaveNum++;
	}
	...
	CvPoint tKeyPoint = {0};
	m_pFlatWeldRealTimeTrack->getKeyPoint(tKeyPoint);
	if (!GetLocalDebugMark())
	{
		...
		fprintf(pfFile, "%d %4d%5d %11.3lf%11.3lf%11.3lf %11.3lf%11.3lf%11.3lf%11.3lf%11.3lf%11.3lf%11.3lf%11.3lf%11.3lf %10d%10d%10d%10d%10d%10d%10d%10d%10d\n",
			nImageNo, tKeyPoint.x, tKeyPoint.y,
	...
	pRobotCtrl->m_cLog->Write("[图像处理]:%d 单次处理时间:%dms %d %d",
		nImageNo, XI_clock() - lTime, tKeyPoint.x, tKeyPoint.y);
	return true;
}
...
//	pObj->m_bJudgeEndReturn = false;
//	pObj->m_pRobotDriver->m_cLog->Write("FuncJudgeEnd_ProcessSearch Start");
//	pObj->m_bJudgeEndReturn = pObj->FuncJudgeEnd_ProcessSearch(pObj->m_pRobotDriver, 5000);
...
//{
//	double dSameDisThreshold = PARA_FLAT_TRACK(dSameDisThreshold);//终点相同距离阈值判断（视觉处理 精度高需要减小此阈值 反之需要增大）
...
//	int nMinProcessImageNum = PARA_FLAT_TRACK(nMinProcessImageNum);// 最少多少张图的局部点云才处理一次
//	double dProcessSampDis = 2.0;		// 点云提取结果中点间距
//	int nWaitTimeStep = 100;			// 等待运动检查时间步距
//	bool bIsRunning = false;
//	int nPreImageNoE = 0;
//	int nErrorNum = 0;					// 连续处理出错计数
//	int nErrorThreshold = PARA_FLAT_TRACK(nMaxProcessPointCloudErrorTimes);// 连续处理出错三次结束流程
...
//			int type;
//			COPini opini;
//			opini.SetFileName(DATA_PATH + "SystemPara.ini");
//			opini.SetSectionName("TotalPartType");
//			opini.ReadString("CurChooseType", &type);
//			if (E_LINELLAE == type)
//			{
//				nWeldInfoNum = GetRiserEndPoint((CvPoint3D64f*)pThreeDPoint, nPtnNum, tWeldInfo, tCameraNorm, tPlaneHNorm, tRefPtn, 2, "GetRiserEndPoint-Lap");
//			}
//			else
//			{
//				nWeldInfoNum = GetRiserEndPoint((CvPoint3D64f*)pThreeDPoint, nPtnNum, tWeldInfo, tCameraNorm, tPlaneHNorm, tRefPtn, 2, "GetRiserEndPoint");
//			}
...
//			if ((0 >= nWeldInfoNum || dDis < dProcessSampDis) 
//				&& (nErrorNum <= nErrorThreshold))
//			{
...
bool CScanInitModule::FuncJudgeEnd_ProcessSearch(CRobotDriverAdaptor* pRobotCtrl, int nTimeOut)
{
	...
	if (E_LINELLAE == type)
	{
		m_cPartialPointCloudProcess.setConfigFile("GetRiserEndPoint-Lap");
	}
	else
	{
		m_cPartialPointCloudProcess.setConfigFile("GetRiserEndPoint");
	}
	...
	while (bIsRunning)
	{
		...
		m_MutexTrackPointCloud.lock();
		int nCurImageNum = m_pTraceModel->lnImageNo.size();
		if (nCurImageNum < nMinProcessImageNum || m_pTraceModel->ltPointCloud.size() <= 0)
		{
			m_MutexTrackPointCloud.unlock();
			Sleep(nWaitTimeStep);
			bIsRunning = m_ptUnit->WorldIsRunning();
		...
		// 互斥拷贝出最新数据
		long long lTime = XI_clock();
		int nImageNoS = m_pTraceModel->lnImageNo.front();//起始图号
		int nImageNoE = m_pTraceModel->lnImageNo.back();//结束图号
		int nTotalPtnNum = m_pTraceModel->ltPointCloud.size();//总点数
		std::vector<CvPoint3D64f> vtPointCloud(nTotalPtnNum);//本次处理点云
		int nPtnNum = 0;//点序号
		for (const auto& point : m_pTraceModel->ltPointCloud)
		{
			vtPointCloud[nPtnNum].x = point.x;
			vtPointCloud[nPtnNum].y = point.y;
			vtPointCloud[nPtnNum].z = point.z;
			nPtnNum++;
		}
		T_ROBOT_COORS tScanS = m_pTraceModel->ltCapWorldCoord.front();//起始采图坐标
		T_ROBOT_COORS tScanE = m_pTraceModel->ltCapWorldCoord.back();//结束采图坐标
		m_MutexTrackPointCloud.unlock();

		// 每更新一张图最多只处理一次点云
		if (nImageNoE <= nPreImageNoE) 
		{
			Sleep(nWaitTimeStep);
			continue;
		}
		...
		{
			pRobotCtrl->m_cLog->Write("使用旧方法赋予参考方向，可能有问题");
			m_cPartialPointCloudProcess.setRefStartPtn(tScanS.dX + tScanS.dBX, tScanS.dY + tScanS.dBY, tScanS.dZ);
			m_cPartialPointCloudProcess.setRefEndPtn(tScanE.dX + tScanE.dBX, tScanE.dY + tScanE.dBY, tScanE.dZ);
		}
		else
		{
			m_cPartialPointCloudProcess.setRefStartPtn(tRealScanS.dX + tRealScanS.dBX, tRealScanS.dY + tRealScanS.dBY, tRealScanS.dZ);
			m_cPartialPointCloudProcess.setRefEndPtn(tRealScanE.dX + tRealScanE.dBX, tRealScanE.dY + tRealScanE.dBY, tRealScanE.dZ);
		}
		double dDirAngle = m_ptUnit->GetRobotCtrl()->RzToDirAngle(tScanE.dRZ);
		m_cPartialPointCloudProcess.setCameraNorm(-CosD(dDirAngle), -SinD(dDirAngle), m_ptUnit->GetRobotCtrl()->m_nRobotInstallDir > 0.0 ? -1.0 : 1.0);
		m_cPartialPointCloudProcess.setPlaneHNorm(0.0, 0.0, m_ptUnit->GetRobotCtrl()->m_nRobotInstallDir > 0.0 ? 1.0 : -1.0);

		// 处理
		if (!m_cPartialPointCloudProcess.process(nImageNoS, nImageNoE, vtPointCloud))
		{
			CheckMachineEmg(pRobotCtrl);
			XUI::MesBox::PopOkCancel("[过程搜端点]:点云处理失败! 图号{0}-{1}", nImageNoS, nImageNoE);
			bResult = false;
			break;
		}

		// 获取处理结果
		auto& vtThreePoint = m_cPartialPointCloudProcess.getResult();
		Three_DPoint tNewEndPtn = vtThreePoint.back();
		if (m_pTraceModel->vtPointCloudEndPoints.size() <= 0) // 第一次保存所有处理结果
		{
			for (int i = 0; i < vtThreePoint.size(); i++)
		...
		if (m_bDynamicJudgeEndPoint)
		{
			if (E_JUDGE_END_PROCESS_SEARCH == m_eJudgeEndMethod
				&& (JudgeFindEndPoint(m_pTraceModel->vtPointCloudEndPoints, dSameDisThreshold, nSameEndPtnNumThreshold)))
			{
				pRobotCtrl->m_cLog->Write("[过程搜端点]: JudgeFindEndPoint 发现终点 E_JUDGE_END_PROCESS_SEARCH");
				break;
			}

			if (E_JUDGE_END_KNOW == m_eJudgeEndMethod
				&& JudgeEnd_Know(m_pTraceModel->vtPointCloudEndPoints, m_pTraceModel->m_tRealEndpointCoor))
			{
				pRobotCtrl->m_cLog->Write("[过程搜端点]: JudgeFindEndPoint 发现终点 E_JUDGE_END_KNOW");
				break;
			}
...
bool CScanInitModule::GetEndSectionCoorsNew(CRobotDriverAdaptor* pRobotDriver, T_ROBOT_COORS tEndpointCoor)
{
	...
	//末端非自由端变姿态
	//if (pRobotDriver->m_tCurSeam.tWeldLine.EndPointType > 0)
	{
		double dChangeAngle = PARA_FLAT_TRACK(dChangeAngleOfEndPointRZAfterFindEndPoint);//总共变X度
		int nChangePntCount = PARA_FLAT_TRACK(nChangePntCountOfEndPointRZAfterFindEndPoint);//变化Y个点
		int nLastPntNo = pRobotDriver->m_vtWeldLineInWorldPoints.size() - 1;
		nChangePntCount = nChangePntCount > nLastPntNo ? nLastPntNo : nChangePntCount;
		double dEachChangeAngle = pRobotDriver->m_nRobotInstallDir * dChangeAngle / nChangePntCount;

		WriteLog("使用的变化点数 :%d", nChangePntCount);
		for (size_t i = 0; i < nChangePntCount; i++)
		{
			pRobotDriver->m_vtWeldLineInWorldPoints[nLastPntNo - i].dRZ -= (nChangePntCount - i) * dEachChangeAngle;
			WriteLog("姿态变换点： %.3lf %.3lf %.3lf %.3lf %.3lf %.3lf %.3lf %.3lf",
				pRobotDriver->m_vtWeldLineInWorldPoints[nLastPntNo - i].dX, pRobotDriver->m_vtWeldLineInWorldPoints[nLastPntNo - i].dY,
				pRobotDriver->m_vtWeldLineInWorldPoints[nLastPntNo - i].dZ, pRobotDriver->m_vtWeldLineInWorldPoints[nLastPntNo - i].dRX,
				pRobotDriver->m_vtWeldLineInWorldPoints[nLastPntNo - i].dRY, pRobotDriver->m_vtWeldLineInWorldPoints[nLastPntNo - i].dRZ,
				pRobotDriver->m_vtWeldLineInWorldPoints[nLastPntNo - i].dBX, pRobotDriver->m_vtWeldLineInWorldPoints[nLastPntNo - i].dBY);
		}
	}
...
bool CScanInitModule::JudgeFindEndPoint(std::vector<Three_DPoint>& vtPtns, double dDisThreshold, double nPtnNumThreshold)
{
	...
	int nMinEndPtnNum = 
		nEndPtnNum > PARA_FLAT_TRACK(nSearchEndPntNum) 
		? nEndPtnNum - PARA_FLAT_TRACK(nSearchEndPntNum) 
		: 0;
	Three_DPoint tNewEndPtn = vtPtns.back();
	Three_DPoint tOldEndPtn;
	for (int i = nEndPtnNum - 2; i >= nMinEndPtnNum; i--)
	{
		tOldEndPtn = vtPtns[i];
...
//	T_ABS_POS_IN_BASE tPointAbsCoordInBase;
	...
	if (bDynamicProc)
	{
		m_cPartialPointCloudProcess.init(_T(".\\WeldData\\RobotA\\ScanWeldLine\\"));
		AfxBeginThread(ThreadJudgeEnd_ProcessSearch, this);
	}
...
bool CScanInitModule::DynamicCaptureForEndpoint(
	const std::shared_ptr<image_process::FlatWeldEndpointSearch>& pImageProcess,
	CRobotDriverAdaptor* pRobotDriver, int nCameraNo, int nTimeOut/* = 2000*/)
{
	// 先锁定激光
	pImageProcess->lockLaser();

	auto dPointCloudLengthForFindEndpnt = PARA_FLAT_MEASURE(dPointCloudLengthForFindEndpnt);
	int nErrorTimes = 0;//处理失败的次数
	int nMaxErrorTimes = PARA_FLAT_MEASURE(nGetLaserPointMaxErrorTimes);//处理失败的次数上限
	bool bDynamicProc = E_POINT_CLOUD_PROC_MOMENT_DYNAMIC == m_ePointCloudProcMoment ? true : false; // 实时处理 ？ 扫描点云完处理？
	long lStartTime = 0;
	int nCapImageNo = 0;
	T_ROBOT_COORS tRobotCoord; //采图时的直角坐标
	...
	if (!pImageProcess->beforeProcessImage())
	{
		pRobotDriver->m_cLog->Write("DynamicCaptureForVert 初始化处理失败！");
		return false;
	}
	m_sPointCloudFileName = pImageProcess->getPointCloudFileName();
	...
	m_bJudgeEndReturn = true;
	if (bDynamicProc)
	{
		m_cPartialPointCloudProcess.init(_T(".\\WeldData\\RobotA\\SearchEndpoint\\"));
		AfxBeginThread(ThreadJudgeEnd_ProcessSearch, this);
	}
	...
	AfxBeginThread(RobotPoseThread, this);
	// 正在运动中 且 未发现终点
	bool bIsFirstCoord = true;
	while (m_ptUnit->WorldIsRunning())
	{
		...
		// 采图
		long long captureTime = XI_clock();
		if (!pImageProcess->captureImage(1))
		{
			pRobotDriver->m_cLog->Write("DynamicCaptureForVert 采图失败！");
			return false;
		}

		// 显示并存图
		long long lTime1 = XI_clock();
		pImageProcess->showSrcImage();
		pImageProcess->saveSrcImage(nCapImageNo);
		...
		long long lTime2 = XI_clock();
		{
			...
			for (const auto& pose : g_poseBuffer) {
				long long diff = std::abs(pose.timestamp - captureTime);
				if (diff < minDiff) {
					tRobotCoord = pose.coord;
					tRobotPulse = pose.pulse;
					minDiff = diff;
				}
			}
			//m_pRobotDriver->m_cLog->Write("tRobotCoord:%d %d %d", tRobotCoord.dX, tRobotCoord.dY, tRobotCoord.dZ);
		}
		if (g_poseBuffer.size() > 2 && !GetInterpolatedPose(bIsFirstCoord, m_pRobotDriver, captureTime, tRobotCoord, tRobotPulse)) {
			pRobotDriver->m_cLog->Write("无法插值获得坐标！");
			return false;
		}
		bIsFirstCoord = false;
		...
		// 处理
		long long lTime3 = XI_clock();
		CHECK_FALSE_RETURN(pImageProcess->initImageProcessPara(
			nCapImageNo, tRobotPulse, tRobotCoord));
		if (!pImageProcess->processImage())
		{
			nErrorTimes++;
			m_pRobotDriver->m_cLog->Write("失败次数%d", nErrorTimes);
			if (nErrorTimes > nMaxErrorTimes)
			{
				pImageProcess->saveErrorData(true, "搜端点图片处理失败");
		...
		auto& vtTeachResult = pImageProcess->getTeachResult();
		int nSaveNum = vtTeachResult.size();//每张图处理出的点数

		// 更新数据
		long long lTime4 = XI_clock();
		m_MutexTrackPointCloud.lock();
		m_pTraceModel->ltPointCloud.insert(m_pTraceModel->ltPointCloud.end(),
			vtTeachResult.begin(), vtTeachResult.end());
		if(vtTeachResult.size() > 0)
		{
			m_pTraceModel->vtConnerWorldCoords.push_back(
				T_ROBOT_COORS(vtTeachResult.begin()->x,
					vtTeachResult.begin()->y, vtTeachResult.begin()->z,
		...
		// 记录日志
		long long lTime5 = XI_clock();
		pRobotDriver->m_cLog->Write("[立焊扫描]: 扫描采图处理%d 总耗时:%dms 坐标读取:%d "
			"图像采集%d 图像处理:%d 更新数据:%d "
			"CurXYZ:%.3lf %.3lf %.3lf",
			nCapImageNo, lTime5 - lStartTime, lTime3 - lTime2,
			lTime1 - captureTime, lTime4 - lTime3, lTime5 - lTime4,
			tRobotCoord.dX, tRobotCoord.dY, tRobotCoord.dZ);

		// 刷新计数
		nCapImageNo++;
	}
	if (!pImageProcess->afterProcessImage())
	{
		pRobotDriver->m_cLog->Write("DynamicCaptureForVert 完成处理失败！");
		return false;
	}
	m_pRobotDriver->HoldOn();
	Sleep(200);
	...
	//等待终点判断线程结束
	while (bDynamicProc && m_IsOpenJudgeEnd_ProcessSearch)
	{
		Sleep(10);
	}
	if (!m_bJudgeEndReturn)
	{
		XiMessageBox("测量搜端点失败");
	}
	return m_bJudgeEndReturn;
}

bool CScanInitModule::DynamicCaptureForVert(
	const std::shared_ptr<image_process::VertWeldSearch>& pImageProcess, 
	CRobotDriverAdaptor* pRobotDriver, int nTimeOut)
{
	auto dPointCloudLengthForFindEndpnt = PARA_FLAT_MEASURE(dPointCloudLengthForFindEndpnt);
	int nErrorTimes = 0;//处理失败的次数
	int nMaxErrorTimes = PARA_FLAT_MEASURE(nGetLaserPointMaxErrorTimes);//处理失败的次数上限
	...
	while (!m_ptUnit->WorldIsRunning())
	{
		...
		if (nCurWaitTime > nTimeOut)
		{
			pRobotDriver->m_cLog->Write("[立焊扫描]: 等待运动超时！");
			return false;
		}
	}
	...
	AfxBeginThread(RobotPoseThread, this);
	// 正在运动中 且 未发现终点
	bool bIsFirstCoord = true;//是否是第一个坐标，用于初始化时间同步
	while (m_ptUnit->WorldIsRunning())
	{
		// 采图
		lStartTime = XI_clock();
		...
		long long lTime2 = XI_clock();
		{
			...
			for (const auto& pose : g_poseBuffer)
			{
				long long diff = std::abs(pose.timestamp - lStartTime);
				if (diff < minDiff)
				{
					tRobotCoord = pose.coord;
					tRobotPulse = pose.pulse;
		...
		if (g_poseBuffer.size() > 2 
			&& !GetInterpolatedPose(bIsFirstCoord, m_pRobotDriver, lStartTime, tRobotCoord, tRobotPulse))
		{
			pRobotDriver->m_cLog->Write("无法插值获得坐标！");
			return false;
		}
		...
		CHECK_FALSE_RETURN(pImageProcess->initImageProcessPara(
			nCapImageNo, tRobotPulse, tRobotCoord));
		CHECK_FALSE_RETURN(pImageProcess->processImage());
		auto& vtTeachResult = pImageProcess->getTeachResult();
		int nSaveNum = vtTeachResult.size();//每张图处理出的点数
		...
		pRobotDriver->m_cLog->Write("[立焊扫描]: 扫描采图处理%d 总耗时:%dms 坐标读取:%d "
			...
			"CurXYZ:%.3lf %.3lf %.3lf",
			nCapImageNo, lTime5 - lStartTime, lTime3 - lTime2,
			lTime1 - lStartTime, lTime4 - lTime3, lTime5 - lTime4,
			tRobotCoord.dX, tRobotCoord.dY, tRobotCoord.dZ);
...
bool CScanInitModule::lockLaser(CRobotDriverAdaptor* pRobotDriver, int nCameraNo)
{
	...
	int nCurrentSecond = cTime.GetSecond();
	CString strFile;
	strFile.Format("%s%s\\LockImage\\%d-%d-%d-%d-%d-%d.jpg", OUTPUT_PATH, m_ptUnit->GetRobotCtrl()->m_strRobotName, cTime.GetYear(), cTime.GetMonth(), cTime.GetDay(), nCurrentHour, nCurrentMinute, nCurrentSecond);
	SaveImage(m_pShowImage, strFile.GetBuffer());
...
bool CScanInitModule::FuncImageProcess(CRobotDriverAdaptor* pRobotCtrl)
{
	...
	// 新跟踪接口入参
	WidgetLaserInfo* LaserInfo = m_pTraceModel->m_pLaserInfo;
	...
	while (m_IsOpenImageProcess)
	{
		...
		auto bProcRst = WidgetLaserTrack(pImg, LaserInfo, "WidgetLaserTrack");

		// 根据返回结果生成旧接口参考线
		CvPoint LeftPtn, RightPtn;
		if (bProcRst)
		{
			Corner2D = LaserInfo->crossPoint;
			LeftPtn = LaserInfo->verticalPlatePnt;
			RightPtn = LaserInfo->bottomPlatePnt;
			m_pTraceImgProcess->HandlockKeyPoint(Corner2D, LeftPtn, RightPtn, tFrontLine, tBackLine);
		}
		long lTime22 = GetTickCount();

		//pRobotCtrl->m_cLog->Write("WidgetLaserTrack:" + str);
...
{
	XUI::MesBox::PopError("DynamicCaptureNew_H_M已失效");
	return;
//	long lPreStartTime = GetTickCount();
//	long lStartTime = 0;
//	int nCapImageNo = 0;
...
double CScanInitModule::DirAngleToRz(double dDirAngle)
{
	...
	double dRz = dDirAngle;// (dDirAngle - m_dWeldNorAngleInHome) + m_tRobotHomeCoors.dRZ;
    if (dRz>180.0)
    {
        dRz -= 360.0;
    }
...
{
	m_dHandEyeDis = 0.0;
	m_pRobotDriver->m_nRobotInstallDir = PARA_SYSTEM(nRobotInstallDir);
...
void CScanInitModule::RobotPoseTime(CRobotDriverAdaptor* pRobotDrive)
{
	while (m_ptUnit->WorldIsRunning()) {
		long long times = XI_clock(); // 记录时间戳
		T_ROBOT_COORS coord = pRobotDrive->GetCurrentPos_ESTUN();
		if (coord.dX == 0 && coord.dY == 0 && coord.dZ == 0 && coord.dRY == 0)
			coord = pRobotDrive->GetCurrentPos();

		std::vector<T_ANGLE_PULSE> vt;
		pRobotDrive->RobotInverseKinematics(coord, pRobotDrive->m_tTools.tGunTool, vt);
		T_ANGLE_PULSE pulse = vt[0];
		long long times1 = XI_clock(); // 记录时间戳
		TimeStampedPose tsp;
		tsp.timestamp = XI_clock(); // 记录时间戳
		tsp.coord = coord;
		tsp.pulse = pulse;

		{
			std::lock_guard<std::mutex> lock(g_mutex);
			g_poseBuffer.push_back(tsp);
			if (g_poseBuffer.size() > 1000) // 限制缓存大小
				g_poseBuffer.pop_front();
		}
		pRobotDrive->m_cLog->Write("坐标时间：%ld", times1 - times);
		std::this_thread::sleep_for(std::chrono::milliseconds(10)); // 控制刷新频率
	}
}

UINT CScanInitModule::RobotPoseThread(void* pParam)
{
	CScanInitModule* pObj = (CScanInitModule*)pParam;
	//if (!pObj->m_IsOpenRobotPoseTime)
	//{
	//	return 0;
	//}
	//pObj->m_IsOpenRobotPoseTime = true;
	pObj->m_pRobotDriver->m_cLog->Write("RobotPoseTime Start");
	pObj->RobotPoseTime(pObj->m_pRobotDriver);
	pObj->m_pRobotDriver->m_cLog->Write("RobotPoseTime End");
	//pObj->m_IsOpenRobotPoseTime = false;
	return 0;
}

T_ROBOT_COORS CScanInitModule::InterpolateCoord(const TimeStampedPose& pose1, const TimeStampedPose& pose2, long long targetTimestamp)
{
	T_ROBOT_COORS result;

	double ratio = double(targetTimestamp - pose1.timestamp) /
		double(pose2.timestamp - pose1.timestamp);

	result.dX = pose1.coord.dX + ratio * (pose2.coord.dX - pose1.coord.dX);
	result.dY = pose1.coord.dY + ratio * (pose2.coord.dY - pose1.coord.dY);
	result.dZ = pose1.coord.dZ + ratio * (pose2.coord.dZ - pose1.coord.dZ);
	result.dRX = pose1.coord.dRX + ratio * (pose2.coord.dRX - pose1.coord.dRX);
	result.dRY = pose1.coord.dRY + ratio * (pose2.coord.dRY - pose1.coord.dRY);
	result.dRZ = pose1.coord.dRZ + ratio * (pose2.coord.dRZ - pose1.coord.dRZ);

	return result;
}

bool CScanInitModule::FindNearestPosePair(long long targetTime, TimeStampedPose& before, TimeStampedPose& after)
{
	std::lock_guard<std::mutex> lock(g_mutex);

	for (size_t i = 1; i < g_poseBuffer.size(); ++i) {
		if (g_poseBuffer[i - 1].timestamp <= targetTime &&
			g_poseBuffer[i].timestamp >= targetTime)
		{
			before = g_poseBuffer[i - 1];
			after = g_poseBuffer[i];
			return true;
		}
	}
	return false; // 没找到合法区间
}

bool CScanInitModule::GetInterpolatedPose(bool bIsFirst, CRobotDriverAdaptor* pRobotDrive, long long imageTimestamp, T_ROBOT_COORS& interpCoord, T_ANGLE_PULSE& interpPulse)
{
	std::lock_guard<std::mutex> lock(g_mutex);
	...
	if (g_poseBuffer.size() < 2)
	{
		if (!g_poseBuffer.empty())
		{
			interpCoord = g_poseBuffer.back().coord;
			interpPulse = g_poseBuffer.back().pulse;
			return true; // fallback：只返回最近坐标
		}
		else
		{
			return false; // 完全没数据
	...
	for (size_t i = 1; i < g_poseBuffer.size(); ++i)
	{
		const auto& before = g_poseBuffer[i - 1];
		const auto& after = g_poseBuffer[i];

		if (before.timestamp <= imageTimestamp && imageTimestamp <= after.timestamp)
		{
			double ratio = double(imageTimestamp - before.timestamp) /
				double(after.timestamp - before.timestamp);

			// 插值坐标
			interpCoord.dX = before.coord.dX + ratio * (after.coord.dX - before.coord.dX);
			interpCoord.dY = before.coord.dY + ratio * (after.coord.dY - before.coord.dY);
			interpCoord.dZ = before.coord.dZ + ratio * (after.coord.dZ - before.coord.dZ);
			interpCoord.dRX = before.coord.dRX + ratio * xi::math::makesureInCycle<-180, 180>(after.coord.dRX - before.coord.dRX);
			interpCoord.dRY = before.coord.dRY + ratio * xi::math::makesureInCycle<-180, 180>(after.coord.dRY - before.coord.dRY);
			interpCoord.dRZ = before.coord.dRZ + ratio * xi::math::makesureInCycle<-180, 180>(after.coord.dRZ - before.coord.dRZ);

			// 插值脉冲
			//for (int j = 0; j < MAX_AXIS; ++j) {
			//	interpPulse.axis[j] = before.pulse.axis[j] +
			//		ratio * (after.pulse.axis[j] - before.pulse.axis[j]);
			//}
			vector<T_ANGLE_PULSE> vt;
			pRobotDrive->RobotInverseKinematics(interpCoord, pRobotDrive->m_tTools.tGunTool, vt);
			interpPulse = vt[0];
			vt.clear();
	...
	//  第一次若找不到插值区间：使用第一个点（非理想但可接受）
	if (bIsFirst)
	{
		const auto& first = g_poseBuffer[0];
		interpCoord = first.coord;
		interpPulse = first.pulse;
	}
	//  若找不到插值区间：使用最近的一个点（非理想但可接受）
	else
	{
		const auto& last = g_poseBuffer.back();
		interpCoord = last.coord;
		interpPulse = last.pulse;
	}
	return true;
}
