...
bool SingleBoard::WeldSeamGrouping(int& nWeldGroupNum)
{
	...
	for (int nWeldNo = 0; nWeldNo < m_vtWeldSeamInfo.size(); nWeldNo++)
	{
		...
		else
		{
			...
			{
				XUI::MesBox::PopInfo("双侧干涉平焊焊缝{0} 长度小于{1},自动删除！", nWeldNo, PARA_FLAT_MEASURE(dShortSeamThreshold));
				continue;
			}
...
bool SingleBoard::CalcMeasureTrack(int nGroupNo, std::vector<T_ROBOT_COORS>& vtMeasureCoord, std::vector<T_ANGLE_PULSE>& vtMeasurePulse, vector<int>& vnMeasureType, double& dExAxlePos, double& dSafeHeight)
{
	...
	{
		tCoord = GenerateRobotCoord(tPtn, PARA_FLAT_WELD(dWeldPosRX), PARA_FLAT_WELD(dWeldPosRY), DirAngleToRz(dNorAngle + 180.0));
		...
		// 过渡点
		tTempCoord = tCoord; 
		RobotCoordPosOffset(tTempCoord, RzToDirAngle(tTempCoord.dRZ), PARA_SYSTEM(dUpOrDownGunSafeHeight));
	...
	tCoord = GenerateRobotCoord(tPtn, PARA_FLAT_WELD(dWeldPosRX), PARA_FLAT_WELD(dWeldPosRY), DirAngleToRz(dNorAngle));
	DecomposeExAxle(tCoord, dExAxlePos, m_ptUnit->m_nMeasureAxisNo);
	tTeachData.tMeasureCoordGunTool = tCoord;
	m_vtTeachData.push_back(tTeachData);
...
bool SingleBoard::CalcWeldTrack(int nGroupNo)
{
	...
	if (false == GenerateWeldLineData(m_vtTeachResult[m_vtTeachResult.size() - 2].tKeyPtn3D,
		m_vtTeachResult[m_vtTeachResult.size() - 1].tKeyPtn3D, 3, 
		PARA_FLAT_WELD(dWeldPosRX), PARA_FLAT_WELD(dWeldPosRY), dWeldDirAngle, eWeldSeamType, vtWeldCoord))
	{
