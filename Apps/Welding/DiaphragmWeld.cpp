...
void CDiaphragmWeld::GetCircleRealWeldPoint(CRobotDriverAdaptor* pRobotDriver, vector<T_ROBOT_COORS>& realWeldPoint, E_WELD_LINE vcPointOnTheSameLine, double dMachineCarMovePos)
{
	...
	for (int i = 0; i < vcPointOnTheSameLine.vtPoint.size(); i += 3)
	{
		...
		midWledPoint.dRX = PARA_FLAT_WELD(dWeldPosRX);
		midWledPoint.dRY = PARA_FLAT_WELD(dWeldPosRY);
	...
	vector<T_ROBOT_COORS>& vtMeasurePoint, vector<T_ANGLE_PULSE>& vtMeasurePulse, vector<int>& vnMeasureType, double& dExAxis)
{
	...
	if (tVerticalReleInfo.bIsReleveVertical)
	{
		...
		if (tVerticalReleInfo.vvtCorner_Info.at(1).at(1).dBoardLength <
			(m_dHandEyeDis + PARA_FLAT_MEASURE(dMeasureDisThreshold) * 3))
		{
			//关联立焊测量信息
			double dDiff = (tVerticalReleInfo.vvtCorner_Info.at(1).at(1).dBoardLength - m_dHandEyeDis - PARA_FLAT_MEASURE(dMeasureDisThreshold)) / 2;
			if (0 == tVerticalReleInfo.vvtCorner_Info.at(1).at(1).nStartType)
			{
				dDiff = (tVerticalReleInfo.vvtCorner_Info.at(1).at(1).dBoardLength - PARA_FLAT_MEASURE(dMeasureDisThreshold)) / 2;
			}
			if (dDiff > PARA_FLAT_MEASURE(dMeasureDisThreshold))
			{
				dDiff = PARA_FLAT_MEASURE(dMeasureDisThreshold);
			}
			if (dDiff < 10)
			{
			...
			CalcMeasureInBase(tStartPointReleve, PARA_FLAT_MEASURE(dMeasureDisThreshold) + dDiff, 0, dNorAngleReleve, tTempPoint);//关联板起点相邻焊缝测量
			vtMeasurePoint.push_back(tTempPoint);
			vnMeasureType.push_back(E_DOUBLE_LONG_LINE);
			CalcMeasureInBase(tStartPointReleve, PARA_FLAT_MEASURE(dMeasureDisThreshold), 0, dNorAngleReleve, tTempPoint);//关联板起点相邻焊缝测量
			vtMeasurePoint.push_back(tTempPoint);
			vnMeasureType.push_back(E_DOUBLE_LONG_LINE);
		}
		else
		{
			//关联立焊测量信息
			CalcMeasureInBase(tStartPointReleve, PARA_FLAT_MEASURE(dMeasureDisThreshold), 0, dNorAngleReleve, tTempPoint);//关联板面测量
			vtMeasurePoint.push_back(tTempPoint);
			vnMeasureType.push_back(E_DOUBLE_LONG_LINE);
			CalcMeasureInBase(tStartPointReleve, PARA_FLAT_MEASURE(dMeasureDisThreshold) * 2, 0, dNorAngleReleve, tTempPoint);//关联板面测量
			vtMeasurePoint.push_back(tTempPoint);
			vnMeasureType.push_back(E_DOUBLE_LONG_LINE);
	...
	if (tVerticalReleInfo.vvtCorner_Info.at(0).at(0).dBoardLength <
		(m_dHandEyeDis + PARA_FLAT_MEASURE(dMeasureDisThreshold) * 3) || tVerticalReleInfo.vvtCorner_Info.at(0).at(0).dBoardLength < 150)
	{
	...
	{
		CalcMeasureInBase(tStartPoint, -(m_dHandEyeDis + PARA_FLAT_MEASURE(dMeasureDisThreshold) * 2), 0, dNorAngle, tMeasurePointR);//关联板面测量
		vtMeasurePoint.push_back(tMeasurePointR);
		vnMeasureType.push_back(E_DOUBLE_LONG_LINE);
		CalcMeasureInBase(tStartPoint, -(m_dHandEyeDis + PARA_FLAT_MEASURE(dMeasureDisThreshold)), 0, dNorAngle, tMeasurePointR);//关联板面测量
		tTempPoint = tMeasurePointR;
		tTempPoint.dZ = m_dWorkPieceHighTop + 20 * m_nRobotInstallDir;
		vtMeasurePoint.push_back(tMeasurePointR);
		vnMeasureType.push_back(E_DOUBLE_LONG_LINE);
	...
	if (tVerticalReleInfo.vvtCorner_Info.at(0).at(1).dBoardLength <
		(m_dHandEyeDis + PARA_FLAT_MEASURE(dMeasureDisThreshold) * 3))
	{
		double dDiff = (tVerticalReleInfo.vvtCorner_Info.at(0).at(1).dBoardLength - m_dHandEyeDis - PARA_FLAT_MEASURE(dMeasureDisThreshold)) / 2;
		if (0 == tVerticalReleInfo.vvtCorner_Info.at(0).at(1).nStartType)
		{
			dDiff = (tVerticalReleInfo.vvtCorner_Info.at(0).at(1).dBoardLength - PARA_FLAT_MEASURE(dMeasureDisThreshold)) / 2;
		}
		if (dDiff > PARA_FLAT_MEASURE(dMeasureDisThreshold))
		{
			dDiff = PARA_FLAT_MEASURE(dMeasureDisThreshold);
		...
		CalcMeasureInBase(tStartPoint, PARA_FLAT_MEASURE(dMeasureDisThreshold) + dDiff, 0, dNorAngleEnd, tMeasurePointL);//关联板起点相邻焊缝测量
		vtMeasurePoint.push_back(tMeasurePointL);
		vnMeasureType.push_back(E_DOUBLE_LONG_LINE);
		CalcMeasureInBase(tStartPoint, PARA_FLAT_MEASURE(dMeasureDisThreshold), 0, dNorAngleEnd, tMeasurePointL);//关联板起点相邻焊缝测量
		vtMeasurePoint.push_back(tMeasurePointL);
		vnMeasureType.push_back(E_DOUBLE_LONG_LINE);
	...
	{
		CalcMeasureInBase(tStartPoint, PARA_FLAT_MEASURE(dMeasureDisThreshold) * 2, 0, dNorAngleEnd, tMeasurePointL);//关联板起点相邻焊缝测量
		vtMeasurePoint.push_back(tMeasurePointL);
		vnMeasureType.push_back(E_DOUBLE_LONG_LINE);
		CalcMeasureInBase(tStartPoint, PARA_FLAT_MEASURE(dMeasureDisThreshold), 0, dNorAngleEnd, tMeasurePointL);//关联板起点相邻焊缝测量
		vtMeasurePoint.push_back(tMeasurePointL);
	...
	vtMeasurePoint.push_back(tMeasurePointL);
	vnMeasureType.push_back(PARA_FLAT_MEASURE(bImageDirection) ? E_LL_RS_FLIP : E_LS_RL_FLIP);
	...
	, vector<T_ANGLE_PULSE>& vtMeasurePulse, vector<int>& vnMeasureType, double& dExAxisPos, int nIndex, bool bIsStart, T_ANGLE_PULSE* ptPrePulse)
{
	...
	// 先测取起点数据
	int nSIndx = m_dHandEyeDis + PARA_FLAT_MEASURE(dMeasureDisThreshold);
	int nSIndx2 = m_dHandEyeDis + PARA_FLAT_MEASURE(dMeasureDisThreshold) * 2;
	int nSIndx3 = m_dHandEyeDis + PARA_FLAT_MEASURE(dMeasureDisThreshold) * 3;
	// 后测取起点数据
	int nEIndx = nTrackSize2 - PARA_FLAT_MEASURE(dMeasureDisThreshold);
	int nEIndx2 = nTrackSize2 - PARA_FLAT_MEASURE(dMeasureDisThreshold) * 2;
	int nEIndx3 = nTrackSize2 - PARA_FLAT_MEASURE(dMeasureDisThreshold) * 3;
...
//		E_ROBOT_CAR_COORS_ERROR tMoveDir;
		T_ROBOT_COORS dCoor(vtScanData.at(i).x, vtScanData.at(i).y, vtScanData.at(i).z, 
			PARA_FLAT_WELD(dWeldPosRX), PARA_FLAT_WELD(dWeldPosRY), dRZ, 0, 0, 0), dOutCoor;
   ...
   // 若起点为测量得到或被更新得到不可直接跟踪焊接需要获取起始段数据方可焊接（可改变姿态测量靠近起始段数据进行焊接）
	...
	for (int n = 0; n < tBoardInfo.vtCornerRelsve_Info.size(); n++)
	{
		...
		if (0 == tBoardInfo.vtCornerRelsve_Info.at(n).nStartType &&
			tBoardInfo.vtCornerRelsve_Info.at(n).dBoardLength > PARA_FLAT_MEASURE(dMeasureDisThreshold) * 2 + m_dHandEyeDis &&
	...
	if (tBoardInfo.vtCornerRelsve_Info.at(0).bBoardType)//圆弧
	{
		...
		for (int nIdx = 0; nIdx < vcPointOnTheSameLine.vdNormal.size(); nIdx += 3)
		{
			...
			T_ROBOT_COORS tCoor(
				...
				PARA_FLAT_WELD(dWeldPosRX),
				PARA_FLAT_WELD(dWeldPosRY),
...
bool CDiaphragmWeld::GetThoryMeasureTrack(CRobotDriverAdaptor* pRobotDriver, T_BOARD_INFO_NEW& tBoardInfo)
{
	...
	for (int i = 0; i < tBoardInfo.vtCornerRelsve_Info.size(); i++)
	{
		...
		else if (E_MEASURE_PROJPOINT == tBoardInfo.vtBoardPartInfo.at(i).eGetStartMethod)
		{
			...
			if (vtTrack.size() <= m_dHandEyeDis + PARA_FLAT_MEASURE(dMeasureDisThreshold) * 5)
			{
				//已修改
				XUI::MesBox::PopInfo("识别出焊道距离小于：{0:.2f}，无法完成测量", m_dHandEyeDis + PARA_FLAT_MEASURE(dMeasureDisThreshold) * 5);
				//XiMessageBoxOk("识别出焊道距离小于：%.2f，无法完成测量", m_dHandEyeDis + m_dMeasureDisThreshold * 5);
			}
			vtTrack.erase(vtTrack.begin() + PARA_FLAT_MEASURE(dMeasureDisThreshold) * 5 + m_dHandEyeDis, vtTrack.end());
			...
			CHECK_BOOL_RETURN(
				GetMeasureCoors(
					...
					PARA_FLAT_MEASURE(dMeasureDisThreshold)
				)
			);
			CHECK_BOOL_RETURN(
...
bool CDiaphragmWeld::CalcRealWeldTrack(CRobotDriverAdaptor* pRobotDriver, T_BOARD_INFO_NEW& tBoardInfo)
{
	...
	for (int i = 0; i < tBoardInfo.vtCornerRelsve_Info.size(); i++)
	{
			...
			!tBoardInfo.vtBoardPartInfo.at(i).bIsTrackingScanEnd)
		{
			...
			CHECK_BOOL_RETURN(CalcRobotPostureCoors(vtRealWeldCoors, PARA_FLAT_WELD(dWeldPosRX), 
				PARA_FLAT_WELD(dWeldPosRY), 3.0, nStartStepNo, dStChangeAngle, nEndstepNo, dEdChangeAngle,
		...
		if (tBoardInfo.vvtMeasureResult_Warp.at(i).size() > 1)
		{
			...
			*/
			...
			//计算机器人姿态 RX,RY,RZ
			CHECK_BOOL_RETURN(CalcRobotPostureCoors(vtTrackCoors, PARA_FLAT_WELD(dWeldPosRX), 
				PARA_FLAT_WELD(dWeldPosRY), 3.0, nStartStepNo, dStChangeAngle, nEndstepNo, dEdChangeAngle,
				tBoardInfo.vtCornerRelsve_Info.at(i).nStartType > 0, tBoardInfo.vtCornerRelsve_Info.at(i).nEndType > 0, 0, 0,
...
bool CDiaphragmWeld::GetTeachTrackNew(CRobotDriverAdaptor* pRobotDriver, T_BOARD_INFO_NEW& tBoardInfo, int nIndex, vector<T_ROBOT_COORS>& vtTeachTrack, vector<T_ANGLE_PULSE>& vtMeasurePulse, vector<int>& vnMeasureType, T_ANGLE_PULSE* ptPrePulse)
{
	...
	if (nSeamPtnNum < (int)(PARA_FLAT_MEASURE(dMeasureDisThreshold) * 3.0 + m_dHandEyeDis) &&
	...
	else
	{
		...
		{
			nSIdx = (int)(PARA_FLAT_MEASURE(dMeasureDisThreshold) + m_dHandEyeDis);
			nEIdx = (int)(nSeamPtnNum - 2 * PARA_FLAT_MEASURE(dMeasureDisThreshold));

			if (nSeamPtnNum < (int)(PARA_FLAT_MEASURE(dMeasureDisThreshold) * 3.0 + m_dHandEyeDis) &&
				nSeamPtnNum >80 && 0 == tCornerLineInfo.nStartType)
			{
				nSIdx = (int)PARA_FLAT_MEASURE(dMeasureDisThreshold);
				nEIdx = (int)PARA_FLAT_MEASURE(dMeasureDisThreshold) + (nSeamPtnNum - PARA_FLAT_MEASURE(dMeasureDisThreshold)) / 2;
			}
		}
		else
		{
			nSIdx = 0 == tCornerLineInfo.nStartType ? (int)m_dHandEyeDis : m_dHandEyeDis + PARA_FLAT_MEASURE(dMeasureDisThreshold);
			nEIdx = (int)(nSeamPtnNum - PARA_FLAT_MEASURE(dMeasureDisThreshold));
		}
	...
	vector<int>& vnMeasureType, bool bTracking, bool bIsArc, bool bStartType, bool bEndType, double dStepDis, bool bExAxisMoving)
{
	...
	else
	{
		...
		if (!bIsArc)
		{
			nSIdx = bStartType==true? (int)PARA_FLAT_MEASURE(dMeasureDisThreshold) :(int)(m_dHandEyeDis+ PARA_FLAT_MEASURE(dMeasureDisThreshold));
			nSIdx = (int)m_dHandEyeDis-1;
			nEIdx = dStepDis == 100 ? (int)(nSeamPtnNum - 2 * PARA_FLAT_MEASURE(dMeasureDisThreshold)) : (int)m_dHandEyeDis+ PARA_FLAT_MEASURE(dMeasureDisThreshold) * 4-1;
		...
		{
			nSIdx = false == bStartType ? (int)m_dHandEyeDis : m_dHandEyeDis + PARA_FLAT_MEASURE(dMeasureDisThreshold);
			nEIdx = (int)(nSeamPtnNum - PARA_FLAT_MEASURE(dMeasureDisThreshold));
		}
	...
	double& dNormalAngle, int& nStartChangePointNo, int& nEndChangePointNo, double dDisPoint, int nPointNum, bool bCheckDisORNum, int nThresVal, bool bStartChangePosture, bool bEndChangePosture)
{
	...
	for (int i = 0; i < vtOutPoints.size() - 1; i++)
	{
		...
		XI_POINT tPosture = { PARA_FLAT_WELD(dWeldPosRX),PARA_FLAT_WELD(dWeldPosRY),dNormal };
...
bool CDiaphragmWeld::CalcRobotCoorsAccordingTrack(vector<XI_POINT> vtCinPoints, vector<T_ROBOT_COORS>& vtCoutCoors)
{
	...
	for (int i = 0; i < vtCinPoints.size() - 1; i++)
	{
		...
		T_ROBOT_COORS tPosture(vtCinPoints.at(i).x, vtCinPoints.at(i).y, vtCinPoints.at(i).z, 
			PARA_FLAT_WELD(dWeldPosRX), PARA_FLAT_WELD(dWeldPosRY), dNormal, 0, 0, 0);
...
void CDiaphragmWeld::CalcMeasureInBase(XI_POINT tMeasurePoint, double dDis, double dMoveZ, double dAngleVer, T_ROBOT_COORS& tMeasureTrans)
{
	...
	tMeasureTrans.dRX = PARA_FLAT_WELD(dWeldPosRX);
	tMeasureTrans.dRY = PARA_FLAT_WELD(dWeldPosRY);
...
bool CDiaphragmWeld::WeldProcessBefor(CRobotDriverAdaptor* pRobotDriver, vector<T_ROBOT_COORS> vtRealWeldCoors, vector<int> vtRealWeldCoorType, vector<T_ANGLE_PULSE> vtRealWeldPulse, bool bIsTracking)
{
	...
	for (size_t i = nWeldStepNo; i < nWeldPointNum; i++)
	{
		...
		if (bIsTracking && 
			...
			PARA_FLAT_TRACK(dTrackCamHandEyeDis) + vnChangePostureCoor.size() * m_pTraceModel->m_dMoveStepDis < dDis &&
	...
	if (tCoutCoors.size() > 0) // 姿态恢复位置：起弧前锁定
	{
		...
		//起弧前锁定
		if (!g_bLocalDebugMark) {
			CHECK_BOOL_RETURN(m_pScanInit->m_pFlatWeldRealTimeTrack->lockLaser(m_ptUnit->m_bBreakPointContinue));
...
UINT CDiaphragmWeld::ThreadMainContData4Estun(void* pParam)
{
	...
	// ljx 为了兼容新的初始化接口 180 12 无实际意义
	if (!pRobotDriver->EstunTrackInit(startPos, 180, 12)) {
...
{
	auto dGunDownBackSafeDis = PARA_SYSTEM(dUpOrDownGunSafeHeight);
	...
	RobotCoordPosOffset(tStartP, RzToDirAngle(tStartP.dRZ), dGunDownBackSafeDis / 2.0, dGunDownBackSafeDis / 2.0 * m_nRobotInstallDir);
	tCoutCoors.push_back(tStartP);
	RobotCoordPosOffset(tStartP, RzToDirAngle(tStartP.dRZ), -1 * dGunDownBackSafeDis / 2.0, -1 * dGunDownBackSafeDis / 2.0 * m_nRobotInstallDir);
	tCoutCoors.push_back(tStartP);
...
void CDiaphragmWeld::SetWeldTlyParam(T_WELD_PARA tWeldPara)
{
	...
	double dFlatHorComp = m_mdFlatHorComp[405];
	double dFlatHeightComp = m_mdFlatHeightComp[405];
...
bool CDiaphragmWeld::AddSafeDownGunPos(vector<T_ROBOT_COORS>& vCoors, vector<int>& vnCoorType, double dAdjustDis, double dPieceHight)
{
	...
	tCoorSafe.dZ = dPieceHight + PARA_SYSTEM(dUpOrDownGunSafeHeight) * m_pRobotDriver->m_nRobotInstallDir;
...
void CDiaphragmWeld::openTrack(CRobotDriverAdaptor* pRobotDriver, int nCurMovePntNo, bool bIsTracking, int& nNextOpenTrackStep)
{
	// 开启跟踪
	bool bIsOpenTrackAgain = false;
	E_MEASURE_POINT_TYPE tPtType = E_WELD_TRACK;
	if (nCurMovePntNo < m_pTraceModel->m_vtWeldLinePointType.size())
	{
		tPtType = (E_MEASURE_POINT_TYPE)m_pTraceModel->m_vtWeldLinePointType.at(nCurMovePntNo);
		if (E_WELD_OPEN_TRACKING == tPtType &&
			nNextOpenTrackStep != nCurMovePntNo)
		{
			nNextOpenTrackStep = nCurMovePntNo;
			bIsOpenTrackAgain = true;
			pRobotDriver->m_cLog->Write("开启跟踪线程:nCountInt22:%d", nCurMovePntNo);
		}
	}
	// 单次跟踪过程只进一次跟踪线程，开启跟踪条件：初次起弧后进入正常焊接姿态或包角结束后再次进入跟踪姿态
	if (bIsTracking
		&& (E_WELD_TRACK & tPtType)
		&& !(E_WELD_TRACK_CHANGE_POSTURE & tPtType)
		&& !m_pScanInit->m_bOpenTrackingStatus
		&& !m_pTraceModel->m_bCameraFindWeldEnd
		|| bIsOpenTrackAgain)
	{
		...
		m_pTraceModel->m_bIfWeldToEndPoint = false;
		m_pTraceModel->m_bIsCloseTrackThread = false;
		// m_pScanInit->m_pTraceModel->bEndpointDataProcess = false;
		m_ptUnit->SwitchIO("TrackLaser", true);
		Sleep(50);
		AfxBeginThread(m_pScanInit->ThreadRealTimeTracking, m_pScanInit);
		pRobotDriver->m_cLog->Write("开启跟踪线程:nCountInt1:%d", nCurMovePntNo);
	}
}

bool CDiaphragmWeld::DoWeldRuningEstun(T_ROBOT_COORS tIdealEndPnt, int nGroupNo, CRobotDriverAdaptor* pRobotDriver, vector<T_ROBOT_COORS> vtRealWeldCoors, vector<int> vnPointType, bool bIsTracking, E_WELD_SEAM_TYPE eWeldSeamType, bool bIsArc)
{
	// 输入检查&预检 调试模式 初始变量重置
	if (!CheckPreconditions(pRobotDriver))
	{
		return false;
	}
	GetNaturalPop().popInfo("开始焊接运动");
	...
	FILE* RecordRealData;
	// 轨迹/文件初始化
	IniWeldResources(pRobotDriver, strCoutPath, RecordRealData);

	//轨迹总点数
	int nTrackPntCount = pRobotDriver->m_vtWeldLineInWorldPoints.size();
	// 根据工艺需要更改Rz角度  工艺参数界面中设置
	double dRobotWeldAdjustRZ = m_dWeldDipAngle;
	// 初始段点数
	int nFirstNum = pRobotDriver->m_vtWeldLineInWorldPoints.size();
	// 机器人名称
	CString strRobot = pRobotDriver->m_strRobotName;

	//8批量传输时，每次传输最大的传输容量(必须是偶数！！！)（埃斯顿目前每次传输最大的传输容量为10，但是最大速率传输机器人性能不够
	int maxPullNum = PARA_ESTUN(nRobotEachSendPosVarMaxCount);
	//批量传输一批数据后的等待时间
	int batchSendSleepTime = 100; 
	//单个传输点后的等待时间
	//int singleSendSleepTime = 50;
	...
	int count = 0;
	T_ROBOT_COORS tmp;
	T_ROBOT_COORS tLastSendCoord;
	
	T_ROBOT_COORS startPos = pRobotDriver->GetCurrentPos();
	if (!pRobotDriver->EstunTrackInit(startPos, m_pTraceModel->m_tWeldParam.dTrackCurrent, m_pTraceModel->m_tWeldParam.dTrackVoltage, "192.168.60.63", dTotalVal, vec)) {
		XiMessageBox("埃斯顿跟踪接口初始化失败");
		return false;
	}; //初始化跟踪数据
	...
	// 进入跟踪流程 
	// m_pTraceModel->m_bIfWeldToEndPoint判断是否焊接完成
	m_pTraceModel->m_bIsTrackingDataSendOver = false;
	while (false == m_pTraceModel->m_bIfWeldToEndPoint)
	{
		WeldPathPoint.clear();
		...
		//TRACE(" **  当前时间： %ld :第%d个epoch，一共发送%d个WeldPathPoint，和%d个cfg\n", clock(), ++epoch, WeldPathPoint.size(), cfg.size());
		Sleep(batchSendSleepTime);//发完maxPullNum个数据等一会
		if (ncount == (nTrackPntCount))
		{
			XiMessageBox("数据小于19个直接运行job");
			break;
		}
		break;
	}
	...
	// 记录焊接长度
	int nLastStepNo = 0;
	int nChangeRXTimes = 0;
	double dChangeDis = PARA_FLAT_TRACK(dChangeDisOfEndPointRZBeforeFindEndPoint); // 结尾提前调整姿态距离
	double dChangeAngle = PARA_FLAT_TRACK(dChangeAngleOfEndPointRZBeforeFindEndPoint); //结尾提前调整角度
	double dChangeAnglePosToEndPnt = PARA_FLAT_TRACK(dDisBetweenFirstChangeRZPosToIdealEndPnt);//距离理想终点多远开始变姿态
	double dEndChangeAnglePosToEndPnt = PARA_FLAT_TRACK(dDisBetweenLastChangeRZPosToIdealEndPnt);//距离理想终点多远停止变姿态
	double dCurChangeAngle = 0.0;
	int nStepNum = (int)(dChangeDis / m_pTraceModel->m_dMoveStepDis + 0.5);
	double dStepAngle = dChangeAngle / (double)nStepNum * PARA_SYSTEM(nWeldDirection) * pRobotDriver->m_nRobotInstallDir;
	int nChangeNum = 1;
	int nChangeLast = 0;
	T_ROBOT_COORS tLastStepPos = pRobotDriver->m_vtWeldLineInWorldPoints[0];

	while (false == m_pTraceModel->m_bIfWeldToEndPoint || curIndex < (pRobotDriver->m_vtWeldLineInWorldPoints).size() - 1)
	{
		//仅支持单段跟踪
		if (m_pTraceModel->m_bIsTrackingDataSendOver
			&& pRobotDriver->m_vtWeldLineInWorldPoints.size() <= curIndex)
		{
			m_pTraceModel->m_bIfWeldToEndPoint = true;
			break;
		}

		T_ROBOT_COORS tCurCoor = pRobotDriver->GetCurrentPos();

		//找出距离当前位置最近的坐标点序号
		int nNearestNo = -1;
		constexpr double dMaxDis = (SQUARE(3.0));
		double dRealMinDis = 99999.0;
		for (int mvNum = 0; mvNum < pRobotDriver->m_vtWeldLineInWorldPoints.size(); mvNum++)
		{
			auto& tCoord = pRobotDriver->m_vtWeldLineInWorldPoints[mvNum];
			double dDis = SQUARE(tCoord.dX + tCoord.dBX - tCurCoor.dX - tCurCoor.dBX)
				+ SQUARE(tCoord.dY + tCoord.dBY - tCurCoor.dY - tCurCoor.dBY)
				+ SQUARE(tCoord.dZ + tCoord.dBZ - tCurCoor.dZ - tCurCoor.dBZ);
			if (dDis < dMaxDis
				&& dDis < dRealMinDis)
			{
				dRealMinDis = dDis;
		...
		//开启跟踪
		if (nNearestNo >= 0)
			openTrack(pRobotDriver, nNearestNo, bIsTracking, nNextOpenTrackStep);

		// 记录焊接长度
		if (nNearestNo != nLastStepNo && pRobotDriver->m_vtWeldLineInWorldPoints.size() > nNearestNo)
		{
			//todo
			T_ROBOT_COORS tNewPos = pRobotDriver->m_vtWeldLineInWorldPoints[nNearestNo];
			m_pTraceModel->m_dCurrentWeldLen += TwoPointDis(tLastStepPos.dX + tLastStepPos.dBX, tLastStepPos.dY + tLastStepPos.dBY, tLastStepPos.dZ,
				tNewPos.dX + tNewPos.dBX, tNewPos.dY + tNewPos.dBY, tNewPos.dZ);
			nLastStepNo = nNearestNo;
			tLastStepPos = tNewPos;
		}
		if (curIndex < (pRobotDriver->m_vtWeldLineInWorldPoints).size() && curIndex < nNearestNo + 7)
		{
			WeldPathPoint.clear();
			cfg.clear();
			...
			try
			{
				...
				//开始延迟RX变姿态
				// ljx 此处倒挂与正坐rx不一致需要从配置文件中读取 未实现
			/*	if (curIndex < 40)
				{*/
				tmp.dRX = 180.0;
				//}
				/*else if (nChangeRXTimes < m_dPlatWeldRx / 0.5)
				{
					tmp.dRX = 180.0 + nChangeRXTimes * 0.5;
					nChangeRXTimes++;
				}*/
				...
				if (nStepNum > 0)
				{
					double dDis = TwoPointDis(tmp.dX, tmp.dY, tmp.dZ, tIdealEndPnt.dX, tIdealEndPnt.dY, tIdealEndPnt.dZ);
					if (dChangeAnglePosToEndPnt > dDis
						&& dDis > dEndChangeAnglePosToEndPnt)
					{
						dCurChangeAngle -= dStepAngle;
						nStepNum--;
					}
				}
				tmp.dRZ += dCurChangeAngle;

				if (tLastSendCoord.Compare(tmp, T_ROBOT_COORS(0.1, 0.1, 0.1, 0.01, 0.01, 0.01, 0.1, 0.1, 0.1)))//相同坐标不发送
				{
					pRobotDriver->m_mutex.Unlock();
					curIndex++;
					continue;
				...
				WriteLog("第%d个跟踪数据: %lf  dX:%lf, dY:%lf,dZ:%lf,dRX:%lf,dRY:%lf,dRZ:%lf,dBX:%lf,dBY:%lf,dBZ:%lf", count, dCurChangeAngle, tmp.dX, tmp.dY, tmp.dZ, tmp.dRX, tmp.dRY, tmp.dRZ, tmp.dBX, tmp.dBY, tmp.dBZ);
				TRACE(" 当前时间： %ld :第%d个跟踪数据:  dX:%lf, dY:%lf,dZ:%lf,dRX:%lf,dRY:%lf,dRZ:%lf,dBX:%lf,dBY:%lf,dBZ:%lf\n", clock(), count++, tmp.dX, tmp.dY, tmp.dZ, tmp.dRZ, tmp.dRY, tmp.dRX, tmp.dBX, tmp.dBY, tmp.dBZ);
			}
			catch (const std::exception&)
			{
	...
	fclose(m_pScanInit->m_pRecordTheoryPoint);
	T_ROBOT_COORS tEndSafe2 = pRobotDriver->GetCurrentPos();
	RobotCoordPosOffset(tEndSafe2, tEndSafe2.dRZ, 
		PARA_FLAT_WELD(dLiftGunHorizontalDistance), 
		PARA_FLAT_WELD(dLiftGunVerticalDistance)* m_nRobotInstallDir);
	//tEndSafe2.dZ = m_dWorkPieceHighTop + 20 * m_nRobotInstallDir;
	T_ROBOT_MOVE_SPEED tPulseMove(PARA_FLAT_WELD(dLiftGunSpeed), 20, 20);
	vector<T_ROBOT_MOVE_INFO> vtRobotMoveInfo(0);
	vtRobotMoveInfo.push_back(pRobotDriver->PVarToRobotMoveInfo(0, tEndSafe2, tPulseMove, MOVL));
	pRobotDriver->SetMoveValue(vtRobotMoveInfo);
	pRobotDriver->CallJob("CONTIMOVANY");
...
bool CDiaphragmWeld::CheckPreconditions(CRobotDriverAdaptor* pRobotDriver)
{ 
	// 调试模式
	if (g_bLocalDebugMark)
	{
...
bool CDiaphragmWeld::IniWeldResources(CRobotDriverAdaptor* pRobotDriver, CString strCoutPath, FILE*& RecordRealData)
{
	RecordRealData = fopen(strCoutPath + "实际焊接数据.txt", //  断电续焊使用a+
		m_ptUnit->m_bBreakPointContinue ? "a+" : "w");
	m_pScanInit->m_pRecordTheoryPoint = fopen(strCoutPath + "理论焊接数据.txt", "w"); Sleep(50);
	if (m_ptUnit->m_bBreakPointContinue)
	{
		SaveRobotCoor(m_pScanInit->m_pRecordTheoryPoint, m_pTraceModel->m_vtBreakContinueBeforPoints, m_pTraceModel->m_vtBreakContinueBeforPointType);// 保存断续前理论数据
