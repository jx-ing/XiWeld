...
namespace WAM
{
	...
	public:
		...
		virtual void CalcCorvrAngle(bool bBelongStart, bool bBelongPtnFree, bool bOtherPtnFree, T_ROBOT_COORS& tCoord, double dSeamLen = 0.0);

		/********************* 线程及线程函数 *********************/
		bool WaitAndCheckThreadExit(CWinThread* pWinThread, CString sThreadName);
		static UINT ThreadTeachProcess(void* pParam);
		bool TeachProcess(int nTimeOut = 15000); // 超过nTimeOut毫秒没有示教图 超时退出
...
public:
	...
	UINT ThreadMeasure(void* pParam);
