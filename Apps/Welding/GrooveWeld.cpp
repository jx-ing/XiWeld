	...
	const vector<int>& vnPtnType, const T_WELD_PARA& tWeldPara)
{
	...
	tDownCoord.dX += (PARA_SYSTEM(dUpOrDownGunSafeHeight) / 2 * CosD(dNorAngle/*RzToDirAngle(tDownCoord.dRZ)*/));
	tDownCoord.dY += (PARA_SYSTEM(dUpOrDownGunSafeHeight) / 2 * SinD(dNorAngle/*RzToDirAngle(tDownCoord.dRZ)*/));
	...
	tBackCoord.dX += (PARA_SYSTEM(dUpOrDownGunSafeHeight) / 2 * CosD(dNorAngle/*RzToDirAngle(tBackCoord.dRZ)*/));
	tBackCoord.dY += (PARA_SYSTEM(dUpOrDownGunSafeHeight) / 2 * SinD(dNorAngle/*RzToDirAngle(tBackCoord.dRZ)*/));
...
bool GrooveWeld::CalcMeasureTrack_G(int nGroupNo, std::vector<T_ROBOT_COORS>& vtMeasureCoord, std::vector<T_ANGLE_PULSE>& vtMeasurePulse, vector<int>& vnMeasureType, double& dSafeHeight)
{
	...
	for (int nOffsetIdx = 0; (false == bCalcSuccess) && (nOffsetIdx < vdExAxleOffset.size()); nOffsetIdx++)
	{
		...
		// 添加收枪 下枪坐标
		dSafeHeight = dPartMaxHeight + (PARA_SYSTEM(dUpOrDownGunSafeHeight) * (double)m_nRobotInstallDir);
		if (false == AddSafeCoord_G(vtTempCoord, vnTempType, dPartMaxHeight, PARA_SYSTEM(dUpOrDownGunSafeHeight), m_eWeldSeamType))
		{
			WriteLog("ExAxleOffset:%3lf AddSafeCoord Fail!", vdExAxleOffset[nOffsetIdx]);
			continue;
		}
	...
	const vector<int>& vnPtnType, const T_WELD_PARA& tWeldPara)
{
	...
	{
		tDownCoord.dY += PARA_SYSTEM(dUpOrDownGunSafeHeight);
	}
	tDownCoord.dZ += (/*dMaxHeight - tDownCoord.dBZ + */PARA_SYSTEM(dUpOrDownGunSafeHeight) * ((double)m_nRobotInstallDir));
	vtAccurateWeldCoord.push_back(tDownCoord);
	...
	if (E_STAND_GROOVE == m_eWeldSeamType)
	{
		tBackCoord.dY += PARA_SYSTEM(dUpOrDownGunSafeHeight);
	}
	tBackCoord.dZ += (/*dMaxHeight - tBackCoord.dBZ + */PARA_SYSTEM(dUpOrDownGunSafeHeight) * ((double)m_nRobotInstallDir));
	vtAccurateWeldCoord.push_back(tBackCoord);
