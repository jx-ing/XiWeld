...
void GenericWeld::SetRecoParam()
{
	...
	case E_FLANGE_GENERIC:
		opini.SetSectionName("E_FLANGE_GENERIC");
		opini.ReadString("ParamValue0", &m_dZPallet);
		break;
	default: 
		XUI::MesBox::PopInfo("GenericWeld不支持工件类型!"); 
		break;
	}
	opini.SetSectionName("E_LINELLAE");
	opini.ReadString("ParamValue1", &m_dMaxWeldLen);
...
bool GenericWeld::PointCloudProcess(bool bUseModel, CvPoint3D64f* pPointCloud/* = NULL*/, int PointCloudSize/* = 0*/)
{
	...
	//线扫
	CString FileName = GetCurLineScanPointCloudFile();
	try
	{
		if (NULL == pPointCloud)
		{
		...
		{
			//使用牛腿替代工字钢，解决立板超出底板时提取焊缝太长的问题
			pLineOrCircularArcWeldingLine = BracketPointCloudToWeldingLines(
				(Three_DPoint*)pPointCloud, PointCloudSize, &nWeldingSeamNumber, m_dZPallet, ".\\LocalFiles\\ExLib\\Vision\\ConfigFiles");

			//pLineOrCircularArcWeldingLine = SmallSamplePointCloudToWeldingLines(
			//	(Three_DPoint*)pPointCloud, PointCloudSize, &nWeldingSeamNumber, m_dZPallet,
			//	".\\LocalFiles\\ExLib\\Vision\\ConfigFiles");
		...
		{
			// 李伟吊车梁
			pLineOrCircularArcWeldingLine = BeamPointCloudToWeldingLines(
				(Three_DPoint*)pPointCloud, PointCloudSize, &nWeldingSeamNumber, m_dZPallet, 
				".\\LocalFiles\\ExLib\\Vision\\ConfigFiles");
			SavePointCloudProcessResult((LineOrCircularArcWeldingLine*)pLineOrCircularArcWeldingLine, nWeldingSeamNumber);
			ReleaseWeldingLines(&pLineOrCircularArcWeldingLine);
		...
		else if (E_END_PLATE == m_eWorkPieceType)
		{
			...
			pLineOrCircularArcWeldingLine = PurlinSupportPointCloudToWeldingLines(
				(Three_DPoint*)pPointCloud, PointCloudSize, &nWeldingSeamNumber, 
				m_dZPallet, m_dEndBoardSupportLen, m_dPurlinSupportLen, m_dPurlinLen, m_dEndBoardLen, ".\\LocalFiles\\ExLib\\Vision\\ConfigFiles");
		...
		else if (E_FLANGE_GENERIC == m_eWorkPieceType)
		{
			pLineOrCircularArcWeldingLine = WorkpieceFlangeInsideOutsideWeldingLineExtraction(
				(Three_DPoint*)pPointCloud, PointCloudSize, &nWeldingSeamNumber, m_dZPallet, 
				".\\LocalFiles\\ExLib\\Vision\\ConfigFiles");
			SavePointCloudProcessResult((LineOrCircularArcWeldingLine*)pLineOrCircularArcWeldingLine, nWeldingSeamNumber);
			ReleaseWeldingLines(&pLineOrCircularArcWeldingLine);
...
bool GenericWeld::WeldSeamGrouping(int& nWeldGroupNum)
{
	...
	double dMinFlatSeamLen = m_dHandEyeDis > 0.0 ? m_dHandEyeDis + PARA_FLAT_MEASURE(dMeasureDisThreshold) * 2.0 : 40.0; // 最小平焊长度
	...
	for (int nWeldNo = 0; nWeldNo < m_vtWeldSeamInfo.size(); nWeldNo++)
	{
		...
		if (IsStandWeldSeam(tLineSeam.StartNormalVector)) // 立焊起点终点法向量Z值为0
		{
			...
			if (tLineSeamInfo.tAtrribute.nEndWrapType == 10)
			{
		...
		{
			if (PARA_FLAT_TRACK(bTrackingEnable) && dWeldSeamLen > PARA_FLAT_TRACK(dTrackingLenThreshold)) // 满足跟踪条件？
			{
			...
			// 超长平焊缝单独保存
			if (dWeldSeamLen > PARA_RECOGNITION(dLengthSeamThreshold) ||
				tLineSeamInfo.tAtrribute.nEndWrapType == 10)
			{
				vtWeldLinePlaneOverLength.push_back(tLineSeamInfo);
				continue;
			}
			...
			if (dWeldSeamLen < PARA_FLAT_MEASURE(dShortSeamThreshold) &&
				0 < tLineSeam.StartPointType &&
				0 < tLineSeam.EndPointType)
			{
				//已修改
				XUI::MesBox::PopInfo("双侧干涉平焊焊缝{0} 长度小于{1:.1f},自动删除！", nWeldNo, PARA_FLAT_MEASURE(dShortSeamThreshold));
				//XiMessageBoxOk("双侧干涉平焊焊缝%d 长度小于%.1lf,自动删除！", nWeldNo, PARA_FLAT_MEASURE(dShortSeamThreshold));
				continue;
			}
	...
	// 平焊分组 (每组首尾相连平焊焊缝的索引)
	sort(vvnWeldNo.begin(), vvnWeldNo.end(), [](const vector<int>& vn1, const vector<int>& vn2) -> bool
		{
			return vn1.size() > vn2.size();
		});
...
bool GenericWeld::CalcMeasureTrack(int nGroupNo, std::vector<T_ROBOT_COORS>& vtMeasureCoord, std::vector<T_ANGLE_PULSE>& vtMeasurePulse, vector<int>& vnMeasureType, double& dExAxlePos, double& dSafeHeight)
{
	...
	for (int nOffsetIdx = 0; (false == bCalcSuccess) && (nOffsetIdx < vdExAxleOffset.size()); nOffsetIdx++)
	{
		for (int nOffsetIdx_y = 0; (false == bCalcSuccess) && (nOffsetIdx_y < vdExAxleOffset_y.size()); nOffsetIdx_y++)
		{
			...
			//SaveCoordToFile(vtTempCoord, sFileName);
			// 添加收枪 下枪坐标
			dSafeHeight = dPartMaxHeight + (PARA_SYSTEM(dUpOrDownGunSafeHeight) * (double)m_nRobotInstallDir);
			if (false == AddSafeCoord(vtTempCoord, vnTempType, dPartMaxHeight, PARA_SYSTEM(dUpOrDownGunSafeHeight)))
			{
	...
	for (int nGroupNo = 0; nGroupNo < m_vvtWeldLineInfoGroup.size(); nGroupNo++)
	{
		for (int nSeamNo = 0; nSeamNo < m_vvtWeldLineInfoGroup[nGroupNo].size(); nSeamNo++)
		{
			...
			else
			{
				...
				tCoord = GenerateRobotCoord(tPtn, PARA_FLAT_WELD(dWeldPosRX), PARA_FLAT_WELD(dWeldPosRY), DirAngleToRz(dAdjoinSeamNorAngle));
	...
	double &dExAxlePos, vector<T_ROBOT_COORS>& vtMeasureCoord, vector<int> &vnMeasureType, vector<int>& vtSortIdx)
{
	...
	for (int i = 1; i < vtTempCoord.size(); i++)
	{
		...
		if (fabs(tPreCoord.dRZ - tCurCoord.dRZ) > 100.0) // 相邻测量点Rz相差超过100°
		{
			...
			tTempCoord.dZ = dPartHeight + PARA_SYSTEM(dUpOrDownGunSafeHeight);// (tPreCoord.dZ + tCurCoord.dZ) / 2.0;
			tTempCoord.dRX = PARA_FLAT_WELD(dWeldPosRX);
			tTempCoord.dRY = PARA_FLAT_WELD(dWeldPosRY);
	...
	double& dExAxlePos, vector<T_ROBOT_COORS>& vtMeasureCoord, vector<int>& vnMeasureType, vector<int>& vtSortIdx)
{
	...
	for (int i = 1; i < vtTempCoord.size(); i++)
	{
			...
			(false == JudgeAngle(180.0, -180.0, tPreCoord.dRZ, tCurCoord.dRZ, 45.0))) // 外圈焊缝 Rz变化超过45°
		{
			...
			tTempCoord.dRY = PARA_FLAT_WELD(dWeldPosRY);
			tTempCoord.dRZ = TwoAngleMedian(180.0, -180.0, tPreCoord.dRZ, tCurCoord.dRZ);
			RobotCoordPosOffset(tTempCoord, RzToDirAngle(tTempCoord.dRZ), PARA_FLAT_MEASURE(dMeasureDisThreshold) * 2.0);
			vtMeasureCoord.push_back(tTempCoord);
			vnMeasureType.push_back(E_TRANSITION_POINT);
		}
		else if (false == JudgeAngle(180.0, -180.0, tPreCoord.dRZ, tCurCoord.dRZ, 100.0) ||
			(dDis > 200.0))// 相邻测量点Rz相差超过100°
		{
			...
			tTempCoord.dX = (tPreCoord.dX + tCurCoord.dX) / 2.0;
			tTempCoord.dY = (tPreCoord.dY + tCurCoord.dY) / 2.0;
			tTempCoord.dZ = dPartHeight + (PARA_SYSTEM(dUpOrDownGunSafeHeight) * m_nRobotInstallDir); // (tPreCoord.dZ + tCurCoord.dZ) / 2.0;
			tTempCoord.dRX = PARA_FLAT_WELD(dWeldPosRX);
			tTempCoord.dRY = PARA_FLAT_WELD(dWeldPosRY);
			tTempCoord.dRZ = TwoAngleMedian(180.0, -180.0, tPreCoord.dRZ, tCurCoord.dRZ);
			vtMeasureCoord.push_back(tTempCoord);
...
{
	double dMeasureRX = PARA_FLAT_MEASURE(dMeasurePosRX);
	double dMeasureRY = PARA_FLAT_MEASURE(dMeasurePosRY);
	vtMeasureCoord.clear();
	double dMaxAcuteAngle = PARA_FLAT_MEASURE(dMaxAngleBetweenTwoVerticalPlatesForChangeRZ);
	double dMinAcuteAngle = PARA_FLAT_MEASURE(dMinAngleBetweenTwoVerticalPlates);
	XiAlgorithm alg;
	T_ROBOT_COORS tCoord;
	...
	double dSeamLen = TwoPointDis3D(tSeam.StartPoint, tSeam.EndPoint);
	...
	tCoord = GenerateRobotCoord(tSeam.StartPoint, dMeasureRX, dMeasureRY, DirAngleToRz(dNorAngleS));
	...
	tCoord = GenerateRobotCoord(tSeam.EndPoint, dMeasureRX, dMeasureRY, DirAngleToRz(dNorAngleE));
...
{
	vtMeasureCoord.clear();
	auto dUnfreeEndpointSearchDisNotOnWeldSeam = PARA_FLAT_MEASURE(dUnfreeEndpointSearchDisNotOnWeldSeam);
	auto dMeasurePosRX = PARA_FLAT_MEASURE(dMeasurePosRX);
	auto dMeasurePosRY = PARA_FLAT_MEASURE(dMeasurePosRY);
...
#endif // 0
	//double dHandEyeDis = bOffsetHandEyeDis ? m_dHandEyeDis : 0.0;
	if (bIsStart)
	{
		// 圆弧定长搜起点，待优化
		tCoord = GenerateRobotCoord(tSeam.StartPoint, dMeasurePosRX, dMeasurePosRY, DirAngleToRz(dNorAngleS));
		RobotCoordPosOffset(tCoord, dOffsetDirS, dOfficeDis);//搜索距离
		CalcCorvrAngle(true, !tSeam.StartPointType, !tSeam.EndPointType, tCoord, dSeamLen);
		vtMeasureCoord.push_back(tCoord);

		tCoord = GenerateRobotCoord(tSeam.StartPoint, dMeasurePosRX, dMeasurePosRY, DirAngleToRz(dNorAngleS));
		RobotCoordPosOffset(tCoord, dOffsetDirS, -dUnfreeEndpointSearchDisNotOnWeldSeam);//搜索距离
		CalcCorvrAngle(true, !tSeam.StartPointType, !tSeam.EndPointType, tCoord, dSeamLen);
		vtMeasureCoord.push_back(tCoord);
	}
	else
	{
		tCoord = GenerateRobotCoord(tSeam.EndPoint, dMeasurePosRX, dMeasurePosRY, DirAngleToRz(dNorAngleE));
		RobotCoordPosOffset(tCoord, dOffsetDirE, dOfficeDis);
		CalcCorvrAngle(false, !tSeam.EndPointType, !tSeam.StartPointType, tCoord, dSeamLen);
		vtMeasureCoord.push_back(tCoord);

		tCoord = GenerateRobotCoord(tSeam.EndPoint, dMeasurePosRX, dMeasurePosRY, DirAngleToRz(dNorAngleE));
		RobotCoordPosOffset(tCoord, dOffsetDirE, -dUnfreeEndpointSearchDisNotOnWeldSeam);//搜索距离
		CalcCorvrAngle(false, !tSeam.EndPointType, !tSeam.StartPointType, tCoord, dSeamLen);
		vtMeasureCoord.push_back(tCoord);
...
bool GenericWeld::CalcSeamTeachData(int nGroupNo, int nSeamNo, double dExAxlePos, vector<LineOrCircularArcWeldingLine> vtSeamGroup, vector<T_TEACH_DATA>& vtTeachData)
{
	...
	if (!bIsFlatWeld && 10 == m_vvtWeldLineInfoGroup[nGroupNo][nSeamNo].tAtrribute.nStartWrapType) // 扫描局部点云提取焊缝，仅支持单个焊缝
	{
	...
	double length = TwoPointDis(tSeam.EndPoint.x, tSeam.EndPoint.y, tSeam.EndPoint.z, 
		tSeam.StartPoint.x, tSeam.StartPoint.y, tSeam.StartPoint.z);
	double dSearchDis = PARA_FLAT_MEASURE(dEndpointSearchDis); // 自由端搜索长度距离
	double dSearchDis2 = PARA_FLAT_MEASURE(dEndpointSearchDis2); // 自由端搜索长度距离
	if (length < dSearchDis + 10.0)
	{
		dSearchDis = length / 2 - 15.0;
		dSearchDis2 = length / 2 - 15.0;
	}
	vtTeachData.clear();
	XiAlgorithm alg;
	...
	double dShortSeamThreshold = PARA_FLAT_MEASURE(dShortSeamThreshold); //测量起点终点的测量点的偏移距离
	if (bIsFlatWeld) 
	{
	...
	{
		//double sd = dSearchDis;
		/*if (E_LINELLAE == m_eWorkPieceType && tSeam.EndPoint.x < tSeam.StartPoint.x)
		{
			sd = dSearchDis * 2 + 30.0;
		}*/
		CHECK_BOOL_RETURN(CalcFreeTeachDataFlat(nGroupNo, nSeamNo, bIsFlatWeld, true, tSeam, dNorAngleS, dSearchDis, dSearchDis2, dExAxlePos, vtTempTeachData));
	}
	else if (bIsFlatWeld && !bFreeStartPoint)// 起点 平焊 干涉端点
	{
		...
		// 原有流程
		if (10 == m_vvtWeldLineInfoGroup[nGroupNo][nSeamNo].tAtrribute.nStartWrapType)
		{
			CHECK_BOOL_RETURN(CalcInterfereTeachDataFlatBySearch(nGroupNo, nSeamNo, bIsFlatWeld, true, tSeam, dNorAngleS, dSearchDis, dExAxlePos, vtTempTeachData));
		}
		else
		{
	...
	{
		//已修改7 == m_vvtWeldLineInfoGroup[nGroupNo][nSeamNo].tAtrribute.nEndWrapType
	...
	{
		//double ed = dSearchDis;
	/*	if (E_LINELLAE == m_eWorkPieceType && tSeam.EndPoint.x > tSeam.StartPoint.x)
		{
			ed = dSearchDis * 2 + 30.0;
		}*/
		CHECK_BOOL_RETURN(CalcFreeTeachDataFlat(nGroupNo, nSeamNo, bIsFlatWeld, false, tSeam, dNorAngleE, dSearchDis, dSearchDis2, dExAxlePos, vtTempTeachData));
	}
	else if (bIsFlatWeld && !bFreeEndPoint)		// 终点 平焊 干涉端点
	{
		...
		// 原有流程
		if (10 == m_vvtWeldLineInfoGroup[nGroupNo][nSeamNo].tAtrribute.nEndWrapType)
		{
			CHECK_BOOL_RETURN(CalcInterfereTeachDataFlatBySearch(nGroupNo, nSeamNo, bIsFlatWeld, false, tSeam, dNorAngleS, dSearchDis, dExAxlePos, vtTempTeachData));
		}
		else
		{
...
bool GenericWeld::CalcScanMeasureTeachData(int nGroupNo, int nSeamNo, double dExAxlePos, vector<LineOrCircularArcWeldingLine> vtSeamGroup, vector<T_TEACH_DATA>& vtTeachData)
{
	...
	double dRobotChangeDir = 1 == m_nRobotInstallDir ? 1.0 : -1.0;
	double dScanRz = m_pRobotDriver->DirAngleToRz(dNorAngle);//立缝法向对应的RZ角度
	double dHorizontalOffset = PARA_VER_MEASURE(dLineScanHorizontalOffset);//水平偏移
	double dVerticalOffset = PARA_VER_MEASURE(dLineScanVerticalOffset);//垂直偏移

	//点云识别得到的理论立缝起点
	T_ROBOT_COORS tCoord = GenerateRobotCoord(tStartPtn, PARA_VER_MEASURE(dLineScanPosRX),
		PARA_VER_MEASURE(dLineScanPosRY), dScanRz);

	// 沿着法向偏移
	RobotCoordPosOffset(tCoord, dNorAngle, dHorizontalOffset, dVerticalOffset * m_nRobotInstallDir);

	// 垂直法向偏移，生成起点
	RobotCoordPosOffset(tCoord, dNorAngle + 90.0, dHorizontalOffset, 0.0);
	DecomposeExAxle(tCoord, dExAxlePos);
	tTeachData.tMeasureCoordGunTool = tCoord;
	tTeachData.tMeasureCoordGunTool.dRZ += PARA_VER_MEASURE(dLineScanOffsetRZForFirstTrack);
	vtTeachData.push_back(tTeachData);

	// 垂直法相偏移，生成终点
	RobotCoordPosOffset(tCoord, dNorAngle - 90.0, dHorizontalOffset * 2, 0.0);
	tTeachData.tMeasureCoordGunTool = tCoord;
	tTeachData.tMeasureCoordGunTool.dRZ += PARA_VER_MEASURE(dLineScanOffsetRZForSecondTrack);
	vtTeachData.push_back(tTeachData);
...
bool GenericWeld::CalcSeamArcTeachData(int nGroupNo, int nSeamNo, double dExAxlePos, vector<LineOrCircularArcWeldingLine> vtSeamGroup, vector<T_TEACH_DATA>& vtTeachData)
{
	...
	double dStartEndDis = TwoPointDis3D(tSeamLine.StartPoint, tSeamLine.EndPoint);
...
bool GenericWeld::CalcTeachDataCircle(int nGroupNo, int nSeamNo, double dExAxlePos, vector<LineOrCircularArcWeldingLine> vtSeamGroup, vector<T_TEACH_DATA>& vtTeachData)
{
	...
	double dRadius = TwoPointDis2D(tSeamLine.StartPoint, tSeamLine.CenterPoint);
	...
	{
		tCoord = GenerateRobotCoord(tSeamLine.CenterPoint, PARA_FLAT_WELD(dWeldPosRX), 
			PARA_FLAT_WELD(dWeldPosRY), dFirstDirAngle + (nNo * dAngleInterval * dRobotDir));
		DecomposeExAxle(tCoord, dExAxlePos);
...
bool GenericWeld::CalcTeachDataFreeCurve(int nGroupNo, int nSeamNo, double dExAxlePos, vector<LineOrCircularArcWeldingLine> vtSeamGroup, vector<T_TEACH_DATA>& vtTeachData)
{
	...
	double dNorAngleS = alg.CalcArcAngle(tSeam.StartNormalVector.x, tSeam.StartNormalVector.y);
	double dNorAngleE = alg.CalcArcAngle(tSeam.EndNormalVector.x, tSeam.EndNormalVector.y);
	double dStartEndDis = TwoPointDis3D(tSeam.StartPoint, tSeam.EndPoint);
	...
	else if (bIsFlatWeld && bFreeStartPoint)// 起点 平焊 自由端点
	{
		...
		CHECK_BOOL_RETURN(CalcFreeTeachDataFlat(nGroupNo, nSeamNo, bIsFlatWeld, true, tSeam, dNorAngleS, dSearchLegth, dSearchDis2, dExAxlePos, vtTempTeachData));
	}
	else if (bIsFlatWeld && !bFreeStartPoint)// 起点 平焊 干涉端点
	{
		CHECK_BOOL_RETURN(CalcInterfereTeachDataFlatFix(nSeamNo, bIsFlatWeld, true, vtSeamGroup, dNorAngleS, dExAxlePos, vtTempTeachData));
	...
	{
		XUI::MesBox::PopInfo("起点测量位置计算失败！焊缝类型{0} 起点类型{1}",
			bIsFlatWeld, bFreeEndPoint);
		return false;
	}
	vtTeachData.insert(vtTeachData.end(), vtTempTeachData.begin(), vtTempTeachData.end());
	// 搜索端点次数
	m_vvtWeldLineInfoGroup.at(nGroupNo).at(nSeamNo).tAtrribute.nScanEndpointNum++;

	// 焊缝终点测量数据
	vtTempTeachData.clear();
	m_pScanInit->m_pTraceModel->m_bIsTrackingScanEnd = false;
	if (bTrack || IsCircle) // 跟踪或闭合圆弧不搜索结尾
	{
		vtTempTeachData.clear();
		m_pScanInit->m_pTraceModel->m_bIsTrackingScanEnd = true;
	}
	else if (bIsFlatWeld && bFreeEndPoint) // 终点 平焊 自由端点
	{
		CHECK_BOOL_RETURN(CalcFreeTeachDataFlat(nGroupNo, nSeamNo, bIsFlatWeld, false, tSeam, dNorAngleE, dSearchDis, dSearchDis2, dExAxlePos, vtTempTeachData));
	}
	else if (bIsFlatWeld && !bFreeEndPoint) // 终点 平焊 干涉端点
	...
	{
		XUI::MesBox::PopInfo("终点测量位置计算失败！焊缝类型{0} 终点类型{1}",
			bIsFlatWeld, bFreeStartPoint);
		return false;
	}
	vtTeachData.insert(vtTeachData.end(), vtTempTeachData.begin(), vtTempTeachData.end());
...
bool GenericWeld::CalcFreeTeachDataFlat(int nGroupNo, int nSeamNo, bool bIsFlatSeam, 
	bool bIsStartPtn, LineOrCircularArcWeldingLine tSeam, double dNorAngle, double dSearchDis, double dSearchDis2, double dExAxlePos, vector<T_TEACH_DATA>& vtTeachData)
{
	/*
		int nWeldNo;						// ！测量点所属焊缝编号
		int nMeasurePtnNo;					// 测量点编号(同一个测量点可用于计算多个焊缝端点)
	...
	*/
	...
	tCoord = GenerateRobotCoord(tPtn, PARA_FLAT_MEASURE(dMeasurePosRX),
		PARA_FLAT_MEASURE(dMeasurePosRY), DirAngleToRz(dNorAngle));
	...
	if (bMeasureNeedOffset)
	{
		RobotCoordPosOffset(tCoord, dOffsetDirE, dSearchDis2);
	}
	DecomposeExAxle(tCoord, dExAxlePos);
	CalcCorvrAngle(bIsStartPtn, bMeasureBelongPtnFree, bOtherPtnFree, tCoord, dSeamLen);
	tTeachData.tMeasureCoordGunTool = tCoord;
...
bool GenericWeld::CalcInterfereTeachDataFlatBySearch(int nGroupNo, int nSeamNo, bool bIsFlatSeam, bool bIsStartPtn, LineOrCircularArcWeldingLine tSeam, double dNorAngle, double dSearchDis, double dExAxlePos, vector<T_TEACH_DATA>& vtTeachData)
{
	/*
		int nWeldNo;						// ！测量点所属焊缝编号
		int nMeasurePtnNo;					// 测量点编号(同一个测量点可用于计算多个焊缝端点)
	...
	*/
	...
	tTeachData.eWeldSeamType = bIsFlatSeam ? E_FLAT_SEAM : E_STAND_SEAM;
	tTeachData.eAttribute = bIsStartPtn ? E_BELONG_START : E_BELONG_END;
	tTeachData.eEndpointType = E_FREE_POINT;//实际是干涉端，但是为了后续计算，这里按自由端设置
	CvPoint3D64f tPtn = bIsStartPtn ? tSeam.StartPoint : tSeam.EndPoint;
	double dSeamLen = TwoPointDis3D(tSeam.StartPoint, tSeam.EndPoint);
	...
	bool bMeasureBelongPtnFree = bIsStartPtn ? !tSeam.StartPointType : !tSeam.EndPointType;
	bool bOtherPtnFree = bIsStartPtn ? !tSeam.EndPointType : !tSeam.StartPointType;
	double dOffsetRZ = bIsStartPtn 
		? PARA_FLAT_MEASURE(dStartPointInterfereSearchOffsetRZ)
		: PARA_FLAT_MEASURE(dEndPointInterfereSearchOffsetRZ);
	...
	if (bMeasureNeedOffset)
	{
		RobotCoordPosOffset(tCoord, dOffsetDirE, PARA_FLAT_MEASURE(dUnfreeEndpointSearchDisNotOnWeldSeam));
	}
	DecomposeExAxle(tCoord, dExAxlePos);
	CalcCorvrAngle(bIsStartPtn, bMeasureBelongPtnFree, bOtherPtnFree, tCoord, dSeamLen);
	tCoord.dRZ += dOffsetRZ;
...
bool GenericWeld::CalcInterfereTeachDataFlat(int nSeamNo, bool bIsFlatSeam, bool bIsStartPtn, vector<LineOrCircularArcWeldingLine> vtSeam, double dNorAngle, double dExAxlePos, vector<T_TEACH_DATA>& vtTeachData)
{
	...
	double dShortSeamThreshold = PARA_FLAT_MEASURE(dShortSeamThreshold);
	T_ROBOT_COORS tCoord; 
	...
	double dCurSeamOffsetDis = PARA_FLAT_MEASURE(dMeasureDisThreshold);
	double dAdjoinOffsetDis = PARA_FLAT_MEASURE(dMeasureDisThreshold);
	double dCurSeamLen = TwoPointDis3D(tSeam.StartPoint, tSeam.EndPoint);
	double dRobotChangeDir = 1 == m_nRobotInstallDir ? 1.0 : -1.0;
	double dSEChangeDir = true == bIsStartPtn ? 1.0 : -1.0;
	...
	{
		dAdjoinSeamLen = TwoPointDis3D(ptAdjoinSeam->StartPoint, ptAdjoinSeam->EndPoint);
	...
	else // 干涉端点相邻边无焊缝
	{
		...
		tCoord = GenerateRobotCoord(tPtn, dMeasureRX, dMeasureRY, DirAngleToRz(dAdjoinSeamNorAngle));
		RobotCoordPosOffset(tCoord, dNorAngle, dAdjoinOffsetDis);
		DecomposeExAxle(tCoord, dExAxlePos);
		...
		tCoord = GenerateRobotCoord(tPtn, dMeasureRX, dMeasureRY, DirAngleToRz(dAdjoinSeamNorAngle));
		RobotCoordPosOffset(tCoord, dNorAngle, dAdjoinOffsetDis * 2.0);
		DecomposeExAxle(tCoord, dExAxlePos);
...
bool GenericWeld::CalcInterfereTeachDataFlatFix(int nSeamNo, bool bIsFlatSeam, bool bIsStartPtn, vector<LineOrCircularArcWeldingLine> vtSeam, double dNorAngle, double dExAxlePos, vector<T_TEACH_DATA>& vtTeachData)
{
	...
	tTeachData.eAttribute = bIsStartPtn ? E_BELONG_START : E_BELONG_END;
	tTeachData.eEndpointType = E_INTERFERE_POINT;
	double dCurSeamOffsetDis = PARA_FLAT_MEASURE(dEndpointSearchDis);// 搜索距离
	//double dAdjoinOffsetDis = PARA_FLAT_MEASURE(dEndpointSearchDis);//搜索距离
	//double dCurSeamLen = TwoPointDis3D(tSeam.StartPoint, tSeam.EndPoint);
	double dRobotChangeDir = 1 == m_nRobotInstallDir ? 1.0 : -1.0;
	//double dSEChangeDir = true == bIsStartPtn ? 1.0 : -1.0;
	...
	vector<LineOrCircularArcWeldingLine> vtSeam, double dExAxlePos, vector<T_TEACH_DATA>& vtTeachData)
{
	...
	if (NULL != ptAdjoinSeam1)	// 立焊顺时针方向存在相邻焊缝
	{
		...
		dAdjoinOffsetDis = dAdjoinSeamLen < dShortSeamThreshold ? dAdjoinSeamLen / 3.0 : dAdjoinOffsetDis;
		if (dAdjoinSeamLen > 50.0 * 3.0
			&& dAdjoinOffsetDis < 50.0)
		{
			dAdjoinOffsetDis = 50.0;
		}
	...
	dAdjoinOffsetDis = PARA_FLAT_MEASURE(dMeasureDisThreshold);
	...
	else						// 立焊逆时针方向不存在相邻焊缝
	{
		...
		double dOffsetDir = dNorAngle + (45.0 * dRobotChangeDir);
		tCoord = GenerateRobotCoord(tPtn, dMeasureRX, dMeasureRY, DirAngleToRz(dAdjoinSeamNorAngle));
		RobotCoordPosOffset(tCoord, dOffsetDir, dAdjoinOffsetDis);
		DecomposeExAxle(tCoord, dExAxlePos);
	...
	if (PARA_FLAT_MEASURE(bImageDirection))
	{
		for (int i = 0; i < vtTeachDataStandUpper.size(); i++)
		{
			vtTeachDataStandUpper[i].nMeasureType = E_LL_RS_FLIP | E_TEACH_POINT;
		}
	}
	else
	{
...
{
	//补充测量点距离
	double dPointSpacing = PARA_FLAT_MEASURE(dPointSpacingForLongWeldSeam);
	//平焊缝上测量点间的距离
	double dShortSeamThreshold = PARA_FLAT_MEASURE(dShortSeamThreshold);
	//
	double dMeasureRX = PARA_FLAT_MEASURE(dMeasurePosRX);
	...
	if (temp > (2 * dPointSpacing)) {
		...
		while (flag >= 2) {
			if (flag == 2) {
				...
				tCoord.dRX = dMeasureRX;
				tCoord.dRY = dMeasureRY;
				...
				vtTempCoord.clear();
				CalcSeamMeasureCoord(tSeam, dPointSpacing, dRobotChangeDir, vtTempCoord, NULL, NULL, false);
				vtMeasureCoord.push_back(vtTempCoord[0]);
	...
	for (int i = 0; i < vtMeasureCoord.size(); i++) {
		...
		vtMeasureCoord[i].dRX = dMeasureRX;
...
bool GenericWeld::CalcInterferePoint(const std::vector<T_TEACH_DATA>& tTeachData, T_ALGORITHM_POINT& tAlgPoint)
{
	...
	for (int i = 0; i < tTeachData.size(); i++)
	{
		...
		if (vtTeachResult.back().vtLeftPtns3D.size() < 3)
			return false;
		if (vtTeachResult.back().vtRightPtns3D.size() < 3)
			return false;
	}
	...
	for (int nWeldNo = 0; nWeldNo < m_vvtWeldLineInfoGroup[nGroupNo].size(); nWeldNo++)
	{
		...
		if (!m_vvtWeldLineInfoGroup[nGroupNo][nWeldNo].tAtrribute.bWeldMode)
		{
			...
			for (size_t i = 0; i < m_vvtWeldLineInfoGroup[nGroupNo][nWeldNo].tAtrribute.dThoeryLength / dPtnInterval; i++)
			{
				...
				tPoint.dRX = PARA_FLAT_WELD(dWeldPosRX);
				tPoint.dRY = PARA_FLAT_WELD(dWeldPosRY);
		...
		else
		{
			eWeldSeamType = GetWeldSeamType(tSeam);
			double dStartEndDis = TwoPointDis3D(tSeam.StartPoint, tSeam.EndPoint);
			bool IsCircle = dStartEndDis < 1.0; // 闭合圆弧	
			...
			double dNormal = atan2(tLineParam.dDirY, tLineParam.dDirX) * 180 / 3.1415926 - 90 * m_nRobotInstallDir;
			double dChangeDisS = PARA_FLAT_TRACK(dChangeDisOfStartPointRZ);	// 变姿态距离
			double dChangeDisE = PARA_FLAT_TRACK(dChangeDisOfEndPointRZ);	// 变姿态距离
			double dChangeAngleS = PARA_FLAT_TRACK(dChangeAngleOfStartPointRZ); // 变姿态角度
			double dChangeAngleE = PARA_FLAT_TRACK(dChangeAngleOfEndPointRZ); // 变姿态角度
			...
			else
			{
				if (!IsCircle)
				{
					...
					CalcRobotPostureCoors(vtCoutPoints, PARA_FLAT_WELD(dWeldPosRX), PARA_FLAT_WELD(dWeldPosRY), dStepDis,
						dChangeDisS, dChangeAngleS, dChangeDisE, dChangeAngleE,
						0 != m_vvtWeldLineInfoGroup[nGroupNo][nWeldNo].tWeldLine.StartPointType,
					...
					{
						T_ROBOT_COORS tRoobt(vtTrack.at(nTemp).x, vtTrack.at(nTemp).y, vtTrack.at(nTemp).z,
							PARA_FLAT_WELD(dWeldPosRX), PARA_FLAT_WELD(dWeldPosRY), dRZ, 0, 0, 0);
				...
				else
				{
					...
					CalcRobotPostureCoors(vtCoutPoints, PARA_FLAT_WELD(dWeldPosRX), PARA_FLAT_WELD(dWeldPosRY), dStepDis, 45.0, 45.0, 45.0, 45.0, 0, 0, 0, 0,
		...
		for (int nPtnIdx = 0; nPtnIdx < vtWeldCoors.size(); nPtnIdx++)
		{
			...
			fprintf(pf, "%d %11.3lf %11.3lf %11.3lf %11.3lf %11.3lf %11.3lf %11.3lf  %11.3lf %4d %4d\n", nPtnIdx,
				tCoord.dX, tCoord.dY, tCoord.dZ, tCoord.dRX, tCoord.dRY, tCoord.dRZ, dExPos, m_dTeachExAxlePos_y,eWeldSeamType, vtWeldCoorsType[nPtnIdx]);
		}
		fclose(pf);
	}
	return true;
