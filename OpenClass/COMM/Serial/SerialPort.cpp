#include "stdafx.h"
#include ".\OpenClass\COMM\Serial\SerialPort.h"
...
//
BOOL CSerialPort::InitPort(void* pPortOwner,	// the owner (CWnd) of the port (receives message)
                           ...
                           void (*pFun)(WPARAM buf, LPARAM portNum, void *pObj))	// size to the writebuffer
{
...
//	if (BytesSent != strlen((char*)port->m_nWriteSize))
//	{
		//TRACE("WARNING: WriteFile() error.. Bytes Sent: %d; Message Length: %d\n", BytesSent, strlen((char*)port->m_nWriteSize));
//	}
...
void CSerialPort::ReceiveChar(CSerialPort* port, COMSTAT comstat)
{
	...
	{ 
        ...
        port->SendMsg(RXBuff, (LPARAM) port->m_nPortNr);
	} // end forever loop
...
{		
	...
	strcpy_s(m_szWriteBuffer, sizeof(m_szWriteBuffer) / sizeof(m_szWriteBuffer[0]), string);
...
void CSerialPort::SendMsg( WPARAM buf, LPARAM portNum )
{
    if (NULL != m_pCallBackFun)
    {
        m_pCallBackFun(buf, portNum, m_pOwner);
    }
}
